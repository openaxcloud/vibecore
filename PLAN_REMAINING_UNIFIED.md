...
## ZONE CloudTenant + Project Factory + identités IAM (17/07 preuves, 21/07 reconstruction, session cloud-tenant)

Contrats `docs/parity/DOMAIN_MODEL.md` §3-4 + `IDENTITY…` : avant le 17/07, écrits mais jamais implémentés. Branche `zone/cloud-tenant-factory-iam` (PR, pas de merge sans feu vert). Preuves : `docs/deploy-evidence/2026-07-17-cloud-tenant-factory-iam/` (audit logs Google + lignes DB réelles + extraits transcript étiquetés — ⚠️ code du 17/07 perdu avec le worktree AVANT commit, réécrit à l'identique le 21/07, 38/38 tests verts ; état PROVEN_REVIEW_PENDING, rien de CLOSED).

| Point | 📤 | 💻 | ✅ | Notes |
|---|:---:|:---:|:---:|---|
| CT-1. Modèle CloudTenant (boundaryType, billingPrincipal, ownershipVersion, residencyPolicy, lifecycle) + CloudProjectBinding 1..N (PRIMARY/REGION_SHARD/QUOTA_SHARD/MIGRATION_TARGET, CapacityPolicy quotas+rate) — migration 0079 | ✅ | ✅ branche | ✅ **17/07 live** | I-TEN-1 = UNIQUE DB `gcpProjectId` ; 1 PRIMARY actif/tenant (2e refusé, testé) |
| CT-2. **Négatif : 2 tenants ne partagent jamais un projet** | ✅ | ✅ branche | ✅ **17/07 live** | Service → 409 `TENANT_PROJECT_CONFLICT` ET INSERT direct → refus DB `P2002` |
| CT-3. **Transfert = revoke → verify → regrant explicite** + machine REQUESTED→…→COMPLETED/FAILED | ✅ | ✅ branche | ✅ **17/07 live** | Ancien principal impersoné : avant = permissions détenues ; après = **probe vide + absent policy, révocation 215 s** ; `assertRegrantAllowed` testé tous états ; ownershipVersion 1→2 ; ligne DB `CloudTenantTransfer` avec évidences conservée |
| CT-4. Machines merge/split/suspension/restauration tenant | ✅ | ✅ branche | 🟡 unit-only | Négatifs testés (merge self, binding étranger, suspend bloque bind) ; pas d'IAM à bouger → pas de leg GCP |
| CT-5. Project Factory REQUESTED→…→ACTIVE (+ excursions + suppression réversible) | ✅ | ✅ branche | ✅ **17/07 live** | Projet réel `ecode-proof-b906ss` (859076086179) → ACTIVE ; CreateProject 11:07Z dans les logs GOOGLE ; 11 events DB ; transitions illégales 409 (testé) |
| CT-6. **Teardown : inventaire → orphelin détecté → erasureProof** ; fenêtre 30 j ; IDs jamais réutilisables | ✅ | ✅ branche | ✅ **17/07 live** | ORPHANS_DETECTED pendant survie puis COMPLETE ; DeleteProject **11:32:05Z logs Google = mail Google à la seconde** ; purge refusée fenêtre ouverte (testé) |
| CT-7. **Rate limit folders MESURÉ** (« ne le suppose pas ») | ✅ | — | ✅ **17/07 mesuré** | **5,8/min soutenu (0,097 req/s)**, burst ~9-10 → 429 « Folder V3 create requests per minute » ; sharding <300 (cap 280), folder JAMAIS par tenant ; table 32 requêtes dans transcript-excerpts |
| CT-8. IAM 3 identités, **RuntimeIdentity réutilisée par révisions**, zéro clé persistante, audit impersonations, séparation build≠promotion | ✅ | ✅ branche | ✅ **17/07 live** | **2 révisions → 1 SA** (`rt-demo-app-7b60706b99f4@…`, created2=false, revisionsServed=2 en DB) ; UNIQUE rend l'anti-pattern non-représentable ; clé USER_MANAGED ⇒ IAM_PERSISTENT_KEY_FORBIDDEN (testé) ; client GCP SANS création de clé |
| CT-9. Routes admin + kill-switch `CLOUD_TENANT_FACTORY_ENABLED` + platform-admin + reauth MFA + audit | ✅ | ✅ branche | 🟡 inject-only | 503/403/409/201-200 testés par injection ; pas déployées (pas de merge) |
| CT-12. **GCP-07 : mail Google = preuve externe du teardown ; RESTORING live** | ✅ | ✅ branche | ✅ **17/07 live** | Create/Delete/**Undelete**Project dans les logs Google (11:07 / 11:32:05 / 12:13:17Z) ; fenêtre 30 j confirmée à l'heure près ; ID-reuse → 409 (aussi loggé Google) ; restauration ~52 s via service ; prod vérifiée ACTIVE 17+21/07 ; ⚠️ logs projet soft-deleted ILLISIBLES → sink org = précondition beta |
| CT-10. Câblage `ensureRuntimeIdentity` dans server-deploy réel | ⬜ | ⬜ | ⬜ | Follow-up flag-gated |
| CT-11. Org Policy baseline, KMS/CMEK, Asset Inventory complet, **log sinks org (précondition beta)**, billing export, VPC-SC/PSC, Essential Contacts, budgets, enforcement CapacityPolicy, interdiction projets hors-factory | ⬜ | ⬜ | ⬜ | Non implémentés — enregistrés honnêtement |
| CT-13. **Provisioning GCP par l'API en prod** (WI + rôles org du GSA plateforme) | ⬜ | ⬜ | ⬜ | **Dépend d'un accès qu'Avi doit autoriser** ; preuves 17/07 = credentials owner en local ; kill-switch OFF en prod |
