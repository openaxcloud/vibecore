...
| ROLLBACK-atomicité. READY↔manifeste fail-closed sur les **10** mutations atteignant READY (`sealPendingRollback`), flip `rollbackable:true` seulement après manifeste durable, reconciler de réparation | ✅ | ⬜ **PR #94 non mergée** | 🟡 local | 2 refus experts. Deux formes de fail-open corrigées : `rollbackable` ABSENT (reconcileDeploymentStatus, publish serveur, rollbacks) et HÉRITÉ à `true` (promote-to-production, rollback-to-deployment — sans crash requis). Tests à crash injecté sur les **vraies** transitions (`CrashAfterCommitStore` : commit puis store DEAD, sinon les `try/catch` des handlers exécutent des compensations qu'un `kill -9` ne fait pas). `docs/deploy-evidence/2026-08-07-rollback-ready-atomicity/` |
| ROLLBACK-concurrence. Chaîne `sélection N-1 → restauration → manifeste → READY` linéarisée par **compare-and-set** sur la tête du flux (`appendReleaseManifestAtHead`) ; concurrents refusés `409 ROLLBACK_RELEASE_MOVED` | ✅ | ⬜ **PR #94 non mergée** | 🟡 local | 3e refus expert : la sélection tournait hors verrou → 3 rollbacks concurrents restauraient tous v1. Pas de verrou de bout en bout (pool advisory dédié à 5 connexions, appel manager borné à 200 s). Preuves : rouge sans fix (`[201,201,201]`), **5/5 PostgreSQL** sur les 2 entrelacements dont contre-exemple `restored===[1,1,1]`, rollback **live avec vrai publish** (`verified==restored==sha256:34083315…`). Suites 77/77 + 142/142, `tsc` 0. `docs/deploy-evidence/2026-08-12-rollback-concurrency/` |
| ROLLBACK-prod. Rollback live **en PRODUCTION** au SHA exact | ⬜ | ⬜ | ⬜ | **Décision d'Avi** : suppose de déployer une branche non mergée d'un lot sensible sur la prod. Non fait, et non faisable unilatéralement |
