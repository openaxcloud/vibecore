...
lives in access-policy tests too, so deleting the interlock from the
...
#### Durable policy enforcement on every supported serving edge (SEC-11–14)

Access is no longer inferred from mutable `Deployment.metadata`. The authority is
the append-only `DeploymentAccessPolicy` row pinned by `Deployment.accessPolicyVersion`
and copied into each `ReleaseManifest`. The preview proxy asks the API for an exact
policy verdict before serving either static or server content; an absent, corrupt,
unknown or mismatched policy is `locked`, never public.

The owner route is `PUT /projects/:projectId/deployments/:deploymentId/access`
with an expected policy version. Passwords are hashed, proofs are bound to the
immutable policy revision, and rotation invalidates prior cookies. Workspace-only
and invite-only policies use short-lived, one-shot exchange tickets and revalidate
membership/grants at serving time. Providers whose public edge cannot enforce the
policy are refused before a deployment or release is published.

Publish, redeploy and rollback never copy a mutable password blob. They must pin
the exact policy version in the deployment and release transaction; a stale or
missing pin fails before promotion. This is what preserves the owner's latest
intent across successor releases and project transfer. The compatibility
`/static-deployments/:id/*` origin only serves protected bytes to the authenticated
preview proxy; a direct anonymous request cannot bypass the dedicated edge.
...
# App side: fail-closed activation, immutable policy/cookie binding, and the
# real shared-cache transition against buildApiApp().
pnpm --filter @vibecore/api test -- \
  src/tests/deployment-access.spec.ts \
  src/tests/deployment-cache-window.spec.ts
