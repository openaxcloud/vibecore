...
# TEARDOWN idempotent ET SÛR si le projet n'existe pas encore (garde
# `gcloud projects describe` : rien à supprimer si le create n'a jamais eu lieu).
TEARDOWN_DONE=0
teardown(){
  local rc=$?
  ...
  { echo "# TEARDOWN $(date -u +%FT%TZ) (exit rc=$rc)"
    if gcloud projects describe "$PROJECT" >/dev/null 2>&1; then
      ...
      echo "PROJECT_STATE=ABSENT (projet '$PROJECT' inexistant — rien à nettoyer, teardown sûr)"
    fi
