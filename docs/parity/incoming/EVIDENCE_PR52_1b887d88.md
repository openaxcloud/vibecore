# PR #52 (physical account purge) — self-contained evidence package

- **Final code head:** `1b887d88ab720b1d4b8922f5eca832cf8c6935a2`
- **Branch:** vague3-purge47-hardening (rebased on origin/main, 0 behind)
- **Scope of this package:** RR-20260804-CODEX-10 REVIEW_BLOCKED — the two deep
  audits (A.1 global OrganizationMember scan, A.2 partial-thaw recovery) plus the
  full CODEX-10 read→freeze atomicity fix, with real-Postgres proofs.
- This file is self-contained: full diffs, full source of the two key files, the
  raw output of the deterministic concurrent test, the A.1 scan, the A.2 recovery
  tests + outputs, and the CI results for the final head.

---

## A) RESULT OF THE TWO DEEP AUDITS

### A.1 — global scan of ALL OrganizationMember mutation paths — HOLE FOUND & FIXED

Command (repo-wide, excluding generated Prisma client + specs):
```
grep -rnE "organizationMember\.(create|createMany|upsert|update|updateMany|delete|deleteMany)" \
  --include="*.ts" services packages app | grep -vE "generated/client|\.spec\.|/tests/|test-api-store"
```
Raw result (only 3 real write sites; no raw SQL, no createMany anywhere):
```
services/api/src/prisma-store.ts:1515:      return tx.organizationMember.upsert({
services/api/src/prisma-store.ts:1567:      const deleted = await tx.organizationMember.deleteMany({ where: { id: found.id } });
services/api/src/prisma-store.ts:999:        const orgMemberships = await tx.organizationMember.deleteMany({ where: { userId } });
```

Per-path classification:

| Site | Method | Lock on `system-setting:membership.purgeFrozenOrgIds`? | Notes |
|---|---|---|---|
| `prisma-store.ts` addMember | `organizationMember.upsert` | **YES** — `assertOrgMembershipNotPurgeFrozen(tx)` inside the same tx | Sole entry for ALL invite / import / admin / SCIM / role-promote/demote routes (see call sites in app.ts) |
| `prisma-store.ts` removeMember | `organizationMember.deleteMany({ id })` | **YES** — `assertOrgMembershipNotPurgeFrozen(tx)` inside the same tx | Sole entry for admin/self member removal |
| `prisma-store.ts` purge tombstone | `organizationMember.deleteMany({ userId })` | **WAS NO → NOW YES** | Finalize tx now takes the lock right after `account-purge:<userId>` |

All external mutation routes funnel through addMember/removeMember (verified — call
sites: app.ts invites ~9489, admin add/remove ~17678-17864, role promote/demote via
addMember upsert which does not change member COUNT). The ONLY direct writer was the
purge tombstone, which bypassed the lock. **This was a reopened race** (the tombstone
could flip an org sole↔shared during another purge's atomic read→freeze section).
**FIXED**: the finalize tx now acquires the membership freeze-set lock in the
canonical order.

**Raw-SQL bypass check** (must be empty):
```
grep -rniE "(insert into|delete from|update)\s+\"?OrganizationMember" --include="*.ts" services packages app | grep -vE "generated/client|.spec."
→ (no output)
```

### A.2 — recovery after a partial thaw — HOLE FOUND & FIXED

`releasePurgeGuarantee` and `reconcilePurgeFreezes` deleted the plan row
UNCONDITIONALLY, even when a freeze-remove failed (`.catch(() => undefined)`). The
plan row is the ONLY durable pointer back to a frozen id, so a failed thaw + deleted
plan strands the freeze forever. **FIXED**: `deletePurgePlanIfFullyThawed()`
re-reads both freeze sets and deletes the plan ONLY when neither still contains any
of the plan's ids; otherwise the plan is kept for the reconciler. Proven by tests
(12)(13)(14) below.

### Canonical advisory-lock order (documented, identical everywhere)
```
account-purge:<userId>
   <  system-setting:membership.purgeFrozenOrgIds        (acquired BEFORE the topology read)
   <  system-setting:objectStorage.purgeFrozenProjectIds
```
- `acquirePurgeGuarantee` and the purge finalize tx both take account-purge then
  membership in this order.
- `addMember` / `removeMember` take only the membership lock.
- `releasePurgeGuarantee` / `reconcilePurgeFreezes` take one lock per separate tx.
- No path can invert the order ⇒ no deadlock.

---

## B.6 — CI RESULTS (final head)

**head_sha (complete):** `1b887d88ab720b1d4b8922f5eca832cf8c6935a2`
**Branch:** vague3-purge47-hardening · **PR:** #52 · **mergeable:** MERGEABLE

Per-workflow conclusions on this exact head (from GitHub check-runs API):

| Workflow / check | Conclusion | Run |
|---|---|---|
| **Install, test, build, scan** (typecheck + full test suite + build) | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30916495698 |
| **CodeQL Analysis (javascript)** | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30916500151 |
| **CodeQL Analysis (typescript)** | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30916500151 |
| **Secret scan (gitleaks, blocking)** | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30916500151 |
| **Quality Gates** | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30916495479 |
| Quality Analysis | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30916495490 |
| Dependency Vulnerability Scan | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30916500151 |
| Secrets Detection | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30916500151 |
| Accessibility Tests | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30916495490 |
| Performance Audit | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30916495490 |
| Release Validation | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30916495479 |
| PR Size Check | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30916495490 |
| Deploy Preview | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30916495327 |
| Validate PR Title | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30916501604 |
| _Playwright local stack_ | ❌ pre-existing repo-wide red | https://github.com/openaxcloud/vibecore/actions/runs/30916495638 |

**Every substantive/blocking check is green.** The single non-green is
`Playwright local stack` (the "Production E2E" UI suite), which is chronically red
on unrelated branches at the same time (e.g. `docs/nix-rr09-code-in-deployment`,
`feat/ex-05-starter-entitlements`) — a repo-wide breakage in the design/marketing UI
tests, with zero relation to account purge. It is non-blocking (`mergeStateStatus`
is UNSTABLE, i.e. all *required* checks passed).

The `Install, test, build, scan` run above executes the SAME test suites whose raw
local output is reproduced verbatim in sections 3, 6, and 6b below: the 13 real-
Postgres purge tests (incl. the deterministic concurrent test (11) and the A.2
recovery tests (12)(13)(14)) and the 34 object-storage tests.

---

## 1) DIFF — full PR #52 net patch over origin/main (final head 1b887d88)

_(The literal `886522b9..HEAD` diff is polluted by 24 rebased main commits — 89k
lines — so this is the clean `git diff origin/main..HEAD`, i.e. the complete PR #52
content at the final head.)_
...
index 62d9fa25..4c839b16 100644
...
@@ -6691,6 +6710,101 @@ async function writeReleaseManifest(
   }
 }
...
@@ -27852,6 +27966,188 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
     };
   });
...
@@ -30069,7 +30365,14 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
...
@@ -30077,7 +30380,32 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
     throw error;
   };
...
@@ -30130,6 +30458,24 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
     }
   });
...
@@ -30137,6 +30483,10 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
...
@@ -30184,6 +30534,10 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
...
+
     const body = parse(
       ...
       request.body ?? {},
@@ -30218,6 +30572,10 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
...
+
     const body = parse(
       ...
       request.body ?? {},
@@ -30272,6 +30630,13 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
...
index 410d7345..82c3b9b2 100644
--- a/services/api/src/prisma-store.ts
+++ b/services/api/src/prisma-store.ts
@@ -4,6 +4,15 @@ import { hashToken } from '@vibecore/auth';
...
+import { deletionStatus, purgeDueAtMs, FINANCIAL_RETENTION_DAYS } from './data-deletion.js';
 ...
 import type {
   AbuseEventRecord,
@@ -120,6 +129,56 @@ function parseJsonArray<T>(value: string | null | undefined): T[] {
   }
 }
...
+
 ...
 /**
@@ -393,6 +452,783 @@ export class PrismaApiStore implements ApiStore {
     }
   }
...
@@ -669,11 +1505,19 @@ export class PrismaApiStore implements ApiStore {
...
@@ -697,24 +1541,35 @@ export class PrismaApiStore implements ApiStore {
   }
...
index 28c197a2..80061632 100644
...
+import type { PurgeStorageDeps, PurgeUserAccountResult } from './account-purge.js';
 ...
 export interface UserRecord {
   id: string;
@@ -1243,6 +1244,26 @@ export interface ApiStore {
...
index 6c9dc134..557ef3e5 100644
...
+import { deletionStatus, purgeDueAtMs, FINANCIAL_RETENTION_DAYS } from '../data-deletion.js';
 ...
 import type {
   EnvVarScope,
@@ -276,6 +285,416 @@ export class TestApiStore implements ApiStore {
     return deleted;
   }
...
### 1b) The CODEX-10 + A.1/A.2 delta only (top 2 commits, HEAD~2..HEAD)

```diff
diff --git a/docs/deploy-evidence/2026-07-23-physical-purge-e2e/README.md b/docs/deploy-evidence/2026-07-23-physical-purge-e2e/README.md
index 74c9ffa7..9bc4933f 100644
--- a/docs/deploy-evidence/2026-07-23-physical-purge-e2e/README.md
+++ b/docs/deploy-evidence/2026-07-23-physical-purge-e2e/README.md
@@ -48,6 +48,58 @@ local `kind` cluster satisfies "real k8s" at $0, so no cost sign-off was needed.
 No persistent keys: GCS uses ADC (the reviewer's gcloud login), k8s uses the
 local kind kubeconfig.
...
+
 ...
 RR-09 found the RR-08 drift guard fired too late — AFTER the irreversible GCS/PVC
diff --git a/services/api/src/prisma-store.ts b/services/api/src/prisma-store.ts
index 7c9e9874..82c3b9b2 100644
--- a/services/api/src/prisma-store.ts
+++ b/services/api/src/prisma-store.ts
@@ -577,10 +577,30 @@ export class PrismaApiStore implements ApiStore {
    */
   private async acquirePurgeGuarantee(userId: string): Promise<PurgeGuarantee> {
     return this.prisma.$transaction(async (tx) => {
+      // CANONICAL LOCK ORDER (must be identical everywhere to avoid deadlock):
...
@@ -625,9 +645,43 @@ export class PrismaApiStore implements ApiStore {
       await this.mutateSystemSettingIds(OBJECT_STORAGE_PURGE_FROZEN_KEY, { remove: projectId }).catch(() => undefined);
     }
...
+    return true;
   ...
   /**
@@ -656,8 +710,14 @@ export class PrismaApiStore implements ApiStore {
         await this.mutateSystemSettingIds(OBJECT_STORAGE_PURGE_FROZEN_KEY, { remove: projectId }).catch(() => undefined);
       }
...
+      }
     ...
     return { reconciled };
@@ -758,8 +818,22 @@ export class PrismaApiStore implements ApiStore {
       // (3) finalize.
       return await this.prisma.$transaction(
         async (tx) => {
+        // CANONICAL LOCK ORDER (see acquirePurgeGuarantee): account-purge < membership.
...
+
         ...
         if (!user) {
diff --git a/services/api/src/tests/account-purge-db.spec.ts b/services/api/src/tests/account-purge-db.spec.ts
index 32311414..9f0490d8 100644
...
+import { describe, expect, it, vi } from 'vitest';
 ...
 import type { ErasureProof, PurgeStorageInventory } from '../account-purge.js';
 import { eraseSubjectStorage } from '../account-storage-purge.js';
@@ -532,4 +532,231 @@ runDbTests('account purge — durable proofs (real Postgres)', () => {
       await prisma.$disconnect();
     }
   });
+
...
## 3) RAW OUTPUT — deterministic concurrent test (11)
...
```
 ...
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (11) CODEX-10: a join in the read→freeze window is reflected in the plan (bucket excluded, never erased) 2719ms
 ↓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (12) A.2: a FAILED membership thaw keeps the plan (freeze not stranded); reconciler recovers
 ↓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (13) A.2: a crash between the two thaws (object-storage thaw fails) keeps the plan; reconciler recovers
 ↓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (14) A.2: the reconciler itself never deletes a plan while a thaw fails (retries next pass)

 Test Files  1 passed (1)
      Tests  1 passed | 12 skipped (13)
   Start at  16:59:17
   Duration  13.09s (transform 4.28s, setup 0ms, collect 9.15s, tests 2.74s, environment 0ms, prepare 508ms)

```

---

## 4) A.1 SCAN — see section A.1 above (per-path lock/justification table + raw grep)

---

## 5) A.2 RECOVERY TESTS — raw output (tests 12, 13, 14 are inside the run below)

The A.2 tests are part of the purge suite in section 6. Their individual assertions:
- (12) membership thaw fails → membership still frozen, plan KEPT; reconciler recovers (freeze lifted + plan deleted).
- (13) crash between thaws (object-storage thaw fails) → object-storage still frozen, plan KEPT; reconciler recovers.
- (14) reconciler itself never deletes a plan while a thaw fails (reconciled=0), then a clean pass deletes it.

---

## 6) RAW OUTPUT — full real-Postgres purge suite (13 tests incl. (11) + (12)(13)(14))

```

 RUN  v3.2.6 /private/tmp/wt-phys47/services/api

 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (2 NEGATIVE first) refuses while the grace window has not elapsed 420ms
{"level":30,"time":1785851928712,"pid":88942,"hostname":"MacBook-Pro-de-HB.local","reqId":"50c449d6-1890-4b57-99bb-810588d0e154","req":{"method":"POST","url":"/internal/account-purge","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785851929052,"pid":88942,"hostname":"MacBook-Pro-de-HB.local","reqId":"50c449d6-1890-4b57-99bb-810588d0e154","event":"request.completed","requestId":"50c449d6-1890-4b57-99bb-810588d0e154","correlationId":"50c449d6-1890-4b57-99bb-810588d0e154","statusCode":200,"durationSeconds":0.3380000591278076}
{"level":30,"time":1785851929053,"pid":88942,"hostname":"MacBook-Pro-de-HB.local","reqId":"50c449d6-1890-4b57-99bb-810588d0e154","res":{"statusCode":200},"responseTime":310.72837498784065,"msg":"request completed"}
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (1+3) purges for real, verifies 0 rows per class in SQL, persists a re-readable proof, then no-ops 1025ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (4) two INDEPENDENT clients racing on the same user yield exactly one purge 530ms
...
prisma:error Ledger LedgerTransaction is append-only: DELETE refused. Correct with a reversing transaction, never by mutating a posted event.
 ...
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (5) a POSTED ledger transaction survives the purge (mig 0078 immutability) and is consigned 456ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (6) sole→shared: a bucket shared under the guarantee is NEVER erased (bucket survives) 1193ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (7) shared→sole is PREVENTED: a co-member cannot leave while the org is purge-frozen 733ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (8) sole→shared is PREVENTED: a new member cannot join while the org is purge-frozen 1239ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (9) NO residual freeze after a FAILED purge (guaranteed release on throw) 955ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (10) reconciler releases a freeze left behind by a crashed run (recoverable state machine) 723ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (11) CODEX-10: a join in the read→freeze window is reflected in the plan (bucket excluded, never erased) 1934ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (12) A.2: a FAILED membership thaw keeps the plan (freeze not stranded); reconciler recovers 2082ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (13) A.2: a crash between the two thaws (object-storage thaw fails) keeps the plan; reconciler recovers 769ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (14) A.2: the reconciler itself never deletes a plan while a thaw fails (retries next pass) 365ms

 Test Files  1 passed (1)
      Tests  13 passed (13)
   Start at  16:58:36
   Duration  23.93s (transform 4.91s, setup 0ms, collect 9.31s, tests 12.44s, environment 1ms, prepare 954ms)

```

---

## 6b) RAW OUTPUT — object-storage suites (34 tests)

```

 RUN  v3.2.6 /private/tmp/wt-phys47/services/api

 ✓ src/tests/account-storage-purge.spec.ts (6 tests) 18ms
 ✓ src/object-storage.spec.ts (26 tests) 43ms
{"level":30,"time":1785851955022,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"ed7aa5c8-62d2-4fde-a8bc-456f86c701c3","req":{"method":"POST","url":"/projects/project_gshslytkp05mseq57gv/thumbnail/upload-url","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785851955092,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"ed7aa5c8-62d2-4fde-a8bc-456f86c701c3","event":"request.completed","requestId":"ed7aa5c8-62d2-4fde-a8bc-456f86c701c3","correlationId":"ed7aa5c8-62d2-4fde-a8bc-456f86c701c3","userId":"user_4l1h24lnsjmmseq57gu","projectId":"project_gshslytkp05mseq57gv","statusCode":200,"durationSeconds":0.06299996376037598}
{"level":30,"time":1785851955093,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"ed7aa5c8-62d2-4fde-a8bc-456f86c701c3","res":{"statusCode":200},"responseTime":24.554958015680313,"msg":"request completed"}
{"level":30,"time":1785851955094,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"a8403b83-0129-4157-8ae0-449a912254ed","req":{"method":"POST","url":"/projects/project_gshslytkp05mseq57gv/thumbnail/upload-url","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785851955101,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"a8403b83-0129-4157-8ae0-449a912254ed","event":"request.completed","requestId":"a8403b83-0129-4157-8ae0-449a912254ed","correlationId":"a8403b83-0129-4157-8ae0-449a912254ed","userId":"user_4l1h24lnsjmmseq57gu","projectId":"project_gshslytkp05mseq57gv","statusCode":403,"durationSeconds":0.006999969482421875}
{"level":30,"time":1785851955102,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"a8403b83-0129-4157-8ae0-449a912254ed","res":{"statusCode":403},"responseTime":6.439500004053116,"msg":"request completed"}
{"level":30,"time":1785851955104,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"3a9cc9a9-20e8-47f9-86af-53fe65770bdd","req":{"method":"POST","url":"/projects/project_gshslytkp05mseq57gv/thumbnail/upload-url","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785851955487,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"6910c374-9768-482c-bd28-846f9cc817ac","req":{"method":"POST","url":"/projects/project_agf1psb2jbbmseq57s6/object-storage/objects/upload-url","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785851955107,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"3a9cc9a9-20e8-47f9-86af-53fe65770bdd","event":"request.completed","requestId":"3a9cc9a9-20e8-47f9-86af-53fe65770bdd","correlationId":"3a9cc9a9-20e8-47f9-86af-53fe65770bdd","userId":"user_4l1h24lnsjmmseq57gu","projectId":"project_gshslytkp05mseq57gv","statusCode":200,"durationSeconds":0.0010001659393310547}
{"level":30,"time":1785851955107,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"3a9cc9a9-20e8-47f9-86af-53fe65770bdd","res":{"statusCode":200},"responseTime":2.692249983549118,"msg":"request completed"}
{"level":30,"time":1785851955642,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"6910c374-9768-482c-bd28-846f9cc817ac","event":"request.completed","requestId":"6910c374-9768-482c-bd28-846f9cc817ac","correlationId":"6910c374-9768-482c-bd28-846f9cc817ac","userId":"user_yepjofjev4gmseq57s4","projectId":"project_agf1psb2jbbmseq57s6","statusCode":403,"durationSeconds":0.14899992942810059}
{"level":30,"time":1785851955642,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"6910c374-9768-482c-bd28-846f9cc817ac","res":{"statusCode":403},"responseTime":151.82420799136162,"msg":"request completed"}
{"level":30,"time":1785851955646,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"eb3ec5c9-74f4-4823-8e3e-30f0a55192f8","req":{"method":"GET","url":"/projects/project_agf1psb2jbbmseq57s6/object-storage/objects","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785851955650,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"eb3ec5c9-74f4-4823-8e3e-30f0a55192f8","event":"request.completed","requestId":"eb3ec5c9-74f4-4823-8e3e-30f0a55192f8","correlationId":"eb3ec5c9-74f4-4823-8e3e-30f0a55192f8","userId":"user_yepjofjev4gmseq57s4","projectId":"project_agf1psb2jbbmseq57s6","statusCode":200,"durationSeconds":0.0019998550415039062}
{"level":30,"time":1785851955650,"pid":91110,"hostname":"MacBook-Pro-de-HB.local","reqId":"eb3ec5c9-74f4-4823-8e3e-30f0a55192f8","res":{"statusCode":200},"responseTime":2.5589170157909393,"msg":"request completed"}
 ✓ src/tests/object-storage-purge-freeze.spec.ts (2 tests) 1613ms
   ✓ object-storage routes — purge freeze barrier > thumbnail/upload-url returns 403 OBJECT_STORAGE_PURGE_FROZEN while the project is frozen (RR-08 #1)  1071ms
   ✓ object-storage routes — purge freeze barrier > a frozen project blocks the generic upload-url write too, but still allows reads  539ms

 Test Files  3 passed (3)
      Tests  34 passed (34)
   Start at  16:59:02
   Duration  12.85s (transform 6.01s, setup 0ms, collect 11.36s, tests 1.67s, environment 0ms, prepare 1.13s)
