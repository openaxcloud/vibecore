# PR #52 (physical account purge) — evidence package (RR-CODEX-12: live lease + singleton + verified reconcile)

- **Final code head:** `dfbe5c6893306e0f2f5199ed48e105c9f7366b1e`
- **Branch:** vague3-purge47-hardening (rebased on origin/main, 0 behind)
- Self-contained: the lease/singleton schema + migration, the diffs, full source of
  the two key files, the raw real-Postgres output of tests 11 + 15–24 (and the full
  suite + object-storage), and CI for the exact head.

---

## A) MODEL (schema / migration 0084)

Three hardening reserves on the per-plan model:

1. **Live lease (heartbeat).** The owner renews the lease via CAS every
   `renewIntervalMs` (≪ TTL); a failed renewal marks it LOST. A `guard`
   revalidates ownership+lease **before each irreversible delete** (threaded into
   `eraseSubjectStorage`), **before finalize**, and **immediately before the
   tombstone** — after loss, nothing is deleted or finalized.
2. **Per-user singleton + safe reclaim.** `PurgePlan.userId` is UNIQUE (one plan
   per subject); a second purge is refused (PURGE_ALREADY_ACTIVE) while a plan is
   live, else an abandoned one is reclaimed. The reconciler reclaims only plans whose
   lease expired **beyond a grace**, via an **atomic conditional DELETE** guarded by
   `(id, version, still-expired)` — a DELETE serialises on the row, so of two
   concurrent reconcilers EXACTLY ONE wins (no double-reclaim), and the version guard
   refuses to delete a plan the owner renewed. Freezes cascade via FK.
3. **Verified reconcile.** `reclaimedPlanIds` is appended only after a `count===1`
   delete AND the freezes are verified gone; a cleanup failure marks the plan
   RECLAIMING (explicit recoverable state), NOT counted.

Migration `0084_purge_plan_lease_singleton/migration.sql`:
```sql
...
**head_sha (complete):** `dfbe5c6893306e0f2f5199ed48e105c9f7366b1e`
...
|---|---|---|
| **Install, test, build, scan** (typecheck + full test suite + build) | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30996343956 |
| **CodeQL Analysis (javascript)** | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30996343859 |
| **CodeQL Analysis (typescript)** | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30996343859 |
| **Secret scan (gitleaks, blocking)** | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30996343859 |
| **Quality Gates** | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30996343925 |
| Quality Analysis | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30996343968 |
| Dependency Vulnerability Scan | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30996343859 |
| Secrets Detection | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30996343859 |
| Accessibility Tests | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30996343968 |
| Performance Audit | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30996343968 |
| Release Validation | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30996343925 |
| PR Size Check | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30996343968 |
| Deploy Preview | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30996344105 |
| Validate PR Title | ✅ success | https://github.com/openaxcloud/vibecore/actions/runs/30996342175 |
| _Playwright local stack_ | ❌ pre-existing repo-wide red | https://github.com/openaxcloud/vibecore/actions/runs/30996344124 |

Every substantive/blocking check is green on this exact head. The single non-green is
`Playwright local stack` (the "Production E2E" UI suite), chronically red on unrelated
branches — a repo-wide breakage in design/marketing UI tests, unrelated to account
purge, and non-blocking (`mergeStateStatus` UNSTABLE = all required checks passed).

`Install, test, build, scan` runs the same suites whose raw output is in sections 3, 4,
4b: the 20 real-Postgres purge tests (incl. concurrent test 11 and the RR-CODEX-12
lease/singleton/reconcile tests 15–24) and the object-storage suites.

NOTE: the head that carries THIS evidence file adds it + its DOCUMENT_MANIFEST.yaml
entry (docs-only); the account-purge code is byte-identical to `dfbe5c68`.
...
diff --git a/packages/database/prisma/migrations/0084_purge_plan_lease_singleton/migration.sql b/packages/database/prisma/migrations/0084_purge_plan_lease_singleton/migration.sql
new file mode 100644
index 00000000..ae6a7cca
--- /dev/null
+++ b/packages/database/prisma/migrations/0084_purge_plan_lease_singleton/migration.sql
@@ -0,0 +1,8 @@
+-- RR-CODEX-12: live-lease heartbeat state + per-user singleton for PurgePlan.
+
+-- Durable reclaim state: ACTIVE (owner holds) | RECLAIMING (reconciler claimed).
...
index 05e737d8..0921c097 100644
--- a/packages/database/prisma/schema.prisma
+++ b/packages/database/prisma/schema.prisma
@@ -2742,3 +2742,42 @@ model WorkspacePostMortem {
...
+  // RR-CODEX-12: at most ONE plan per subject — the per-user singleton that stops
+  // two workers running two physical purges of the same account.
+  userId         String        @unique
+  ownerToken     String
+  leaseExpiresAt DateTime
+  version        Int           @default(0)
+  // 'ACTIVE' while an owner holds it; 'RECLAIMING' once a reconciler CAS-claims an
+  // abandoned plan for cleanup (durable, so the old owner cannot continue).
...
index 62d9fa25..b96c2deb 100644
...
@@ -27852,6 +27963,190 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
     };
   });
...
@@ -30069,7 +30364,14 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
...
@@ -30077,7 +30379,29 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
     throw error;
   };
...
@@ -30130,6 +30454,24 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
     }
   });
...
@@ -30137,6 +30479,10 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
...
@@ -30184,6 +30530,10 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
...
+
     const body = parse(
       ...
       request.body ?? {},
@@ -30218,6 +30568,10 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
...
+
     const body = parse(
       ...
       request.body ?? {},
@@ -30272,6 +30626,13 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
...
index 410d7345..4acbd722 100644
...
+import { deletionStatus, purgeDueAtMs, FINANCIAL_RETENTION_DAYS } from './data-deletion.js';
 ...
 import type {
   AbuseEventRecord,
@@ -120,6 +130,80 @@ function parseJsonArray<T>(value: string | null | undefined): T[] {
   }
 }
...
+
 ...
 /**
@@ -246,6 +330,17 @@ export class PrismaApiStore implements ApiStore {
...
@@ -393,6 +488,896 @@ export class PrismaApiStore implements ApiStore {
     }
   }
...
@@ -669,11 +1654,19 @@ export class PrismaApiStore implements ApiStore {
...
@@ -697,24 +1690,35 @@ export class PrismaApiStore implements ApiStore {
   }
...
index 28c197a2..4ff1f2fe 100644
...
index 6c9dc134..740457fa 100644
...
### 1b) The RR-CODEX-12 delta only (this round's commit, source-only)

```diff
diff --git a/docs/deploy-evidence/2026-07-23-physical-purge-e2e/README.md b/docs/deploy-evidence/2026-07-23-physical-purge-e2e/README.md
index 8fc1d915..e093e78a 100644
...
+
 ...
 The reviewer found a real hole in the freeze model: freezes were GLOBAL id-lists,
diff --git a/docs/parity/DOCUMENT_MANIFEST.yaml b/docs/parity/DOCUMENT_MANIFEST.yaml
index 64685a3d..d49c3c92 100644
--- a/docs/parity/DOCUMENT_MANIFEST.yaml
+++ b/docs/parity/DOCUMENT_MANIFEST.yaml
@@ -2,7 +2,7 @@
...
-fileCount: 173
+fileCount: 172
 generatedAt: "2026-08-05T09:57:38Z"
 generatedFromCommit: b02534235b94e4ca3e5ff12e5b9f0df8bc8d2ce9
 mergedCommit: b02534235b94e4ca3e5ff12e5b9f0df8bc8d2ce9
@@ -418,11 +418,6 @@ documents:
     schemaVersion: 2
     repoCommit: null
     reviewer: UNKNOWN
-  - file: incoming/EVIDENCE_PR52_531f2461.md
-    sha256: "00061e74e020904c5038294532c0ecbceead01d4de4dfc524c7b5c5f348e278f"
-    schemaVersion: null
-    repoCommit: null
-    reviewer: UNKNOWN
   - file: incoming/PR37_REVIEW_CHECKS_20260721.txt
     sha256: "49c255f3dc96e47628c53daa9644b9bdf0309b81b66915584b9f461f2c214c5c"
     schemaVersion: null
diff --git a/packages/database/prisma/migrations/0084_purge_plan_lease_singleton/migration.sql b/packages/database/prisma/migrations/0084_purge_plan_lease_singleton/migration.sql
new file mode 100644
index 00000000..ae6a7cca
...
+CREATE UNIQUE INDEX "PurgePlan_userId_key" ON "PurgePlan"("userId");
diff --git a/packages/database/prisma/schema.prisma b/packages/database/prisma/schema.prisma
index 7270c541..0921c097 100644
--- a/packages/database/prisma/schema.prisma
+++ b/packages/database/prisma/schema.prisma
@@ -2754,14 +2754,18 @@ model WorkspacePostMortem {
 // it never touches a live plan.
 model PurgePlan {
   id             String        @id @default(cuid())
-  userId         String
...
diff --git a/services/api/src/account-purge.ts b/services/api/src/account-purge.ts
index 006a43fd..4a8fc061 100644
--- a/services/api/src/account-purge.ts
+++ b/services/api/src/account-purge.ts
...
+  ) => Promise<{ classes: PurgeClassReport[]; verified: boolean }>;
 ...
 /** Outcome of one purgeUserAccount attempt (store layer). */
diff --git a/services/api/src/account-storage-purge.ts b/services/api/src/account-storage-purge.ts
index f96606ae..29e842b6 100644
--- a/services/api/src/account-storage-purge.ts
+++ b/services/api/src/account-storage-purge.ts
...
diff --git a/services/api/src/app.ts b/services/api/src/app.ts
index 3804d87d..b96c2deb 100644
--- a/services/api/src/app.ts
+++ b/services/api/src/app.ts
@@ -28064,7 +28064,7 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
              * stamped. A failure throws → the account stays queued and is retried
              * → an account is never marked purged with storage still on disk.
              */
...
+            eraseStorage: (inventory, guard) =>
               options.accountStoragePurger
                 ? options.accountStoragePurger(inventory, userId)
                 : eraseSubjectStorage(
@@ -28086,6 +28086,8 @@ export async function buildApiApp(options: ApiAppOptions = {}): Promise<FastifyI
                       // the store's topology guarantee (RR-09); this barrier now
                       // only freezes the workspaces.
                       writeBarrier: createWriteBarrier((inv) => inv.workspaceIds),
+                      // RR-CODEX-12: revalidate the lease before each irreversible delete.
...
diff --git a/services/api/src/prisma-store.ts b/services/api/src/prisma-store.ts
index 0b32960a..4acbd722 100644
--- a/services/api/src/prisma-store.ts
+++ b/services/api/src/prisma-store.ts
@@ -172,21 +172,38 @@ const OBJECT_STORAGE_RESOURCE = 'objectStorage';
 // Advisory-lock name that serialises the membership guarantee's read→freeze with
 // addMember/removeMember (see CANONICAL LOCK ORDER in acquirePurgeGuarantee).
 const MEMBERSHIP_FREEZE_LOCK = 'purge:membership-freeze';
...
+
 ...
 /**
@@ -313,6 +330,17 @@ export class PrismaApiStore implements ApiStore {
      * Node resolver in production.
      */
     private readonly resolveTxt: (hostname: string) => Promise<string[][]> = dnsPromises.resolveTxt,
+
...
+    },
   ...
   async ping(): Promise<void> {
@@ -569,15 +597,37 @@ export class PrismaApiStore implements ApiStore {
    */
   private async acquirePurgeGuarantee(userId: string): Promise<PurgeGuarantee> {
     const ownerToken = randomUUID();
...
+      });
       ...
       const freezeRows = [
         ...topology.orgIds.map((orgId) => ({
@@ -600,6 +650,8 @@ export class PrismaApiStore implements ApiStore {
         planId: plan.id,
         ownerToken,
         userId,
+        version: plan.version,
+        leaseExpiresAt: plan.leaseExpiresAt,
         ...
         orgIds: topology.orgIds,
         bucketProjectIds: topology.bucketProjectIds,
@@ -608,6 +660,93 @@ export class PrismaApiStore implements ApiStore {
     });
   }
...
+
    ...
    * RR-1bd27929: release the guarantee — delete ONLY THIS plan's freeze rows (never
    * another plan's, so a shared resource stays frozen while another live plan owns
@@ -636,34 +775,61 @@ export class PrismaApiStore implements ApiStore {
   }
...
+    return { reconciled: reclaimedPlanIds.length, reclaimedPlanIds };
   ...
   /**
@@ -713,6 +879,12 @@ export class PrismaApiStore implements ApiStore {
     // touched, so there is no guarantee to acquire, drift to guard, or freeze to
     // release.
     let guarantee: PurgeGuarantee | null = null;
+    // RR-CODEX-12: the live-lease heartbeat + the lease-revalidation guard. The
...
+    let guard: (() => Promise<void>) | null = null;
     ...
     if (deps?.eraseStorage) {
       const pre = await this.prisma.user.findUnique({ where: { id: userId }, select: { preferences: true } });
@@ -739,10 +911,33 @@ export class PrismaApiStore implements ApiStore {
     try {
       // (2) erase ONLY under an acquired guarantee, on its locked inventory.
       if (guarantee && deps?.eraseStorage) {
-        const erasure = await deps.eraseStorage({
...
+        );
         ...
         if (!erasure.verified) {
           throw new Error(
@@ -752,6 +947,8 @@ export class PrismaApiStore implements ApiStore {
         }
...
+        await guard(); // revalidate after the (possibly long) erasure, before finalize
       ...
       const erasedTopologyFingerprint = guarantee?.fingerprint ?? null;
@@ -1062,6 +1259,13 @@ export class PrismaApiStore implements ApiStore {
           models: { Project: sharedProjects },
         });
...
+
         ...
         await tx.user.update({
           where: { id: userId },
@@ -1159,8 +1363,15 @@ export class PrismaApiStore implements ApiStore {
         { timeout: 120_000, maxWait: 20_000 },
       );
     } finally {
+      // RR-CODEX-12: stop the heartbeat FIRST (no renewal races the release).
...
diff --git a/services/api/src/store.ts b/services/api/src/store.ts
index 1be1d640..4ff1f2fe 100644
--- a/services/api/src/store.ts
+++ b/services/api/src/store.ts
@@ -1263,7 +1263,7 @@ export interface ApiStore {
    */
   purgeUserAccount(input: { userId: string; nowMs?: number }, deps?: PurgeStorageDeps): Promise<PurgeUserAccountResult>;
   /** RR-09: release any account-purge freeze left behind by a crashed run. */
...
+  reconcilePurgeFreezes(): Promise<{ reconciled: number; reclaimedPlanIds: string[] }>;
   ...
   findUserByEmail(email: string): Promise<UserRecord | undefined>;
diff --git a/services/api/src/tests/account-purge-db.spec.ts b/services/api/src/tests/account-purge-db.spec.ts
index 24d8fc17..81fcd9d0 100644
...
@@ -850,4 +877,281 @@ runDbTests('account purge — durable proofs (real Postgres)', () => {
     }
   });
...
diff --git a/services/api/src/tests/test-api-store.ts b/services/api/src/tests/test-api-store.ts
index 6f4382e9..740457fa 100644
--- a/services/api/src/tests/test-api-store.ts
+++ b/services/api/src/tests/test-api-store.ts
@@ -293,8 +293,8 @@ export class TestApiStore implements ApiStore {
    * the advisory-locked Postgres implementation.
    */
   /** RR-09: no durable freeze state in the in-memory store, so nothing to reconcile. */
-  async reconcilePurgeFreezes(): Promise<{ reconciled: number }> {
...
## 3) RAW OUTPUT — RR-CODEX-12 lease/singleton/reconcile tests (19–24), real Postgres

```
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (19) erasure longer than the TTL: heartbeat renews, a concurrent reconciler reclaims NOTHING 1326ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (20) dead owner: lease expired, two concurrent reconcilers → exactly one wins, cleanup confirmed 514ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (21) renewal vs reclaim race → exactly one CAS winner (owner keeps a valid lease, or stops) 373ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (22) lease lost mid-erasure: after 1 resource, NO further delete, NO tombstone, recoverable 302ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (23) two workers, same user, real erase path → exactly one plan, one physical execution, one tombstone 598ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (24) reconciler cleanup failure: reconciled stays 0, plan RECLAIMING+recoverable, second pass finishes 269ms
      Tests  20 passed (20)
```

---

## 4) RAW OUTPUT — full real-Postgres purge suite (20 tests, incl. 11 + 15–24)

```

 RUN  v3.2.6 /private/tmp/wt-phys47/services/api

 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (2 NEGATIVE first) refuses while the grace window has not elapsed 391ms
{"level":30,"time":1785924724366,"pid":17011,"hostname":"MacBook-Pro-de-HB.local","reqId":"732db4ad-8fe0-4f4a-a18b-e8a9e271f18e","req":{"method":"POST","url":"/internal/account-purge","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785924724676,"pid":17011,"hostname":"MacBook-Pro-de-HB.local","reqId":"732db4ad-8fe0-4f4a-a18b-e8a9e271f18e","event":"request.completed","requestId":"732db4ad-8fe0-4f4a-a18b-e8a9e271f18e","correlationId":"732db4ad-8fe0-4f4a-a18b-e8a9e271f18e","statusCode":200,"durationSeconds":0.3080000877380371}
{"level":30,"time":1785924724677,"pid":17011,"hostname":"MacBook-Pro-de-HB.local","reqId":"732db4ad-8fe0-4f4a-a18b-e8a9e271f18e","res":{"statusCode":200},"responseTime":265.5851670205593,"msg":"request completed"}
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (1+3) purges for real, verifies 0 rows per class in SQL, persists a re-readable proof, then no-ops 985ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (4) two INDEPENDENT clients racing on the same user yield exactly one purge 430ms
...
prisma:error Ledger LedgerTransaction is append-only: DELETE refused. Correct with a reversing transaction, never by mutating a posted event.
 ...
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (5) a POSTED ledger transaction survives the purge (mig 0078 immutability) and is consigned 310ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (6) sole→shared: a bucket shared under the guarantee is NEVER erased (bucket survives) 533ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (7) shared→sole is PREVENTED: a co-member cannot leave while the org is purge-frozen 403ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (8) sole→shared is PREVENTED: a new member cannot join while the org is purge-frozen 314ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (9) NO residual freeze after a FAILED purge (guaranteed release on throw) 251ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (10) reconciler releases a freeze left behind by a crashed run (recoverable state machine) 234ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (11) CODEX-10: a join in the read→freeze window is reflected in the plan (bucket excluded, never erased) 503ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (15) two plans sharing an org: releasing one keeps the org frozen until the LAST plan releases 468ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (16) reconciler NEVER reclaims a live plan (valid lease), even one blocked in erasure 145ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (17) reconciler reclaims an ABANDONED plan via CAS, releasing ONLY its resources 211ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (18) crash between the two thaws keeps the plan recoverable; reprise idempotent; no OTHER plan touched 435ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (19) erasure longer than the TTL: heartbeat renews, a concurrent reconciler reclaims NOTHING 1326ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (20) dead owner: lease expired, two concurrent reconcilers → exactly one wins, cleanup confirmed 514ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (21) renewal vs reclaim race → exactly one CAS winner (owner keeps a valid lease, or stops) 373ms
 ✓ src/tests/account-purge-db.spec.ts > account purge — durable proofs (real Postgres) > (22) lease lost mid-erasure: after 1 resource, NO further delete, NO tombstone, recoverable 302ms
 ...
 Test Files  1 passed (1)
      Tests  20 passed (20)
   Start at  13:11:59
   Duration  12.60s (transform 1.57s, setup 0ms, collect 2.99s, tests 9.00s, environment 0ms, prepare 100ms)

```

## 4b) RAW OUTPUT — object-storage + storage-purge suites

```

 RUN  v3.2.6 /private/tmp/wt-phys47/services/api

 ✓ src/tests/account-storage-purge.spec.ts (6 tests) 8ms
 ✓ src/object-storage.spec.ts (26 tests) 13ms
{"level":30,"time":1785924736983,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"c496dde2-69a9-438b-8837-4812dcc1fcc8","req":{"method":"POST","url":"/projects/project_wmy9y3ksmomsfxh6f9/thumbnail/upload-url","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785924736992,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"097d678d-eaa1-4916-96c3-95117d4d444b","req":{"method":"GET","url":"/projects/project_hkbkle4dvuamsfxh6fo/object-storage/objects?delimiter=/","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785924736998,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"c496dde2-69a9-438b-8837-4812dcc1fcc8","event":"request.completed","requestId":"c496dde2-69a9-438b-8837-4812dcc1fcc8","correlationId":"c496dde2-69a9-438b-8837-4812dcc1fcc8","userId":"user_x4uvezyoimsfxh6f8","projectId":"project_wmy9y3ksmomsfxh6f9","statusCode":200,"durationSeconds":0.011999845504760742}
{"level":30,"time":1785924736998,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"c496dde2-69a9-438b-8837-4812dcc1fcc8","res":{"statusCode":200},"responseTime":6.3338329792022705,"msg":"request completed"}
{"level":30,"time":1785924736999,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"712cd323-c28c-4585-a023-66cfbb90472a","req":{"method":"POST","url":"/projects/project_wmy9y3ksmomsfxh6f9/thumbnail/upload-url","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785924737001,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"712cd323-c28c-4585-a023-66cfbb90472a","event":"request.completed","requestId":"712cd323-c28c-4585-a023-66cfbb90472a","correlationId":"712cd323-c28c-4585-a023-66cfbb90472a","userId":"user_x4uvezyoimsfxh6f8","projectId":"project_wmy9y3ksmomsfxh6f9","statusCode":403,"durationSeconds":0.0019998550415039062}
{"level":30,"time":1785924737001,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"712cd323-c28c-4585-a023-66cfbb90472a","res":{"statusCode":403},"responseTime":2.066332995891571,"msg":"request completed"}
{"level":30,"time":1785924737001,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"d74b17c2-7bb1-41de-9722-0a3ae0879376","req":{"method":"POST","url":"/projects/project_wmy9y3ksmomsfxh6f9/thumbnail/upload-url","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785924737007,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"097d678d-eaa1-4916-96c3-95117d4d444b","event":"request.completed","requestId":"097d678d-eaa1-4916-96c3-95117d4d444b","correlationId":"097d678d-eaa1-4916-96c3-95117d4d444b","projectId":"project_hkbkle4dvuamsfxh6fo","statusCode":200,"durationSeconds":0.013000011444091797}
{"level":30,"time":1785924737007,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"097d678d-eaa1-4916-96c3-95117d4d444b","res":{"statusCode":200},"responseTime":4.913374990224838,"msg":"request completed"}
{"level":30,"time":1785924737008,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"2f439624-8619-4ec8-b0ff-711f23421616","req":{"method":"POST","url":"/projects/project_hkbkle4dvuamsfxh6fo/object-storage/bucket","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785924737009,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"2f439624-8619-4ec8-b0ff-711f23421616","event":"request.completed","requestId":"2f439624-8619-4ec8-b0ff-711f23421616","correlationId":"2f439624-8619-4ec8-b0ff-711f23421616","projectId":"project_hkbkle4dvuamsfxh6fo","statusCode":200,"durationSeconds":0.0010001659393310547}
{"level":30,"time":1785924737009,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"2f439624-8619-4ec8-b0ff-711f23421616","res":{"statusCode":200},"responseTime":1.6832500100135803,"msg":"request completed"}
{"level":30,"time":1785924737010,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"a3320510-d898-40eb-8eba-995e26c16095","req":{"method":"POST","url":"/projects/project_hkbkle4dvuamsfxh6fo/object-storage/objects/upload-url","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785924737073,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"2adc028e-ce27-4252-8333-a4ac4759465c","req":{"method":"POST","url":"/projects/project_s0r8i1wz4vmsfxh6i4/object-storage/objects/upload-url","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785924737002,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"d74b17c2-7bb1-41de-9722-0a3ae0879376","event":"request.completed","requestId":"d74b17c2-7bb1-41de-9722-0a3ae0879376","correlationId":"d74b17c2-7bb1-41de-9722-0a3ae0879376","userId":"user_x4uvezyoimsfxh6f8","projectId":"project_wmy9y3ksmomsfxh6f9","statusCode":200,"durationSeconds":0}
{"level":30,"time":1785924737002,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"d74b17c2-7bb1-41de-9722-0a3ae0879376","res":{"statusCode":200},"responseTime":0.36637499928474426,"msg":"request completed"}
{"level":30,"time":1785924737083,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"2adc028e-ce27-4252-8333-a4ac4759465c","event":"request.completed","requestId":"2adc028e-ce27-4252-8333-a4ac4759465c","correlationId":"2adc028e-ce27-4252-8333-a4ac4759465c","userId":"user_f0z94yl4g4qmsfxh6i4","projectId":"project_s0r8i1wz4vmsfxh6i4","statusCode":403,"durationSeconds":0.009999990463256836}
{"level":30,"time":1785924737083,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"2adc028e-ce27-4252-8333-a4ac4759465c","res":{"statusCode":403},"responseTime":10.158208012580872,"msg":"request completed"}
{"level":30,"time":1785924737083,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"30b0fb7e-6ee0-407f-987e-4746d7b87295","req":{"method":"GET","url":"/projects/project_s0r8i1wz4vmsfxh6i4/object-storage/objects","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785924737084,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"30b0fb7e-6ee0-407f-987e-4746d7b87295","event":"request.completed","requestId":"30b0fb7e-6ee0-407f-987e-4746d7b87295","correlationId":"30b0fb7e-6ee0-407f-987e-4746d7b87295","userId":"user_f0z94yl4g4qmsfxh6i4","projectId":"project_s0r8i1wz4vmsfxh6i4","statusCode":200,"durationSeconds":0.0010001659393310547}
{"level":30,"time":1785924737084,"pid":19036,"hostname":"MacBook-Pro-de-HB.local","reqId":"30b0fb7e-6ee0-407f-987e-4746d7b87295","res":{"statusCode":200},"responseTime":0.37599998712539673,"msg":"request completed"}
 ✓ src/tests/object-storage-purge-freeze.spec.ts (2 tests) 641ms
   ✓ object-storage routes — purge freeze barrier > thumbnail/upload-url returns 403 OBJECT_STORAGE_PURGE_FROZEN while the project is frozen (RR-08 #1)  558ms
{"level":30,"time":1785924737101,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"d4fdc6d7-4c83-40a3-84a3-206ccf33329e","req":{"method":"GET","url":"/projects/project_xw6uqnat7pomsfxh6iw/object-storage/objects","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785924737188,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"e273e75a-9055-40b0-8aad-3ff6ddda2fa4","req":{"method":"GET","url":"/projects/project_5o0zlz0v0v6msfxh6la/object-storage/objects","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785924737270,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"934a1944-0054-43c4-9d84-631261717609","req":{"method":"GET","url":"/projects/project_eemstd42jycmsfxh6nm/object-storage/objects","hostname":"localhost","remoteAddress":"127.0.0.1"},"msg":"incoming request"}
{"level":30,"time":1785924737036,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"a3320510-d898-40eb-8eba-995e26c16095","event":"request.completed","requestId":"a3320510-d898-40eb-8eba-995e26c16095","correlationId":"a3320510-d898-40eb-8eba-995e26c16095","projectId":"project_hkbkle4dvuamsfxh6fo","statusCode":200,"durationSeconds":0.026000022888183594}
{"level":30,"time":1785924737036,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"a3320510-d898-40eb-8eba-995e26c16095","res":{"statusCode":200},"responseTime":26.201792001724243,"msg":"request completed"}
{"level":30,"time":1785924737102,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"d4fdc6d7-4c83-40a3-84a3-206ccf33329e","event":"request.completed","requestId":"d4fdc6d7-4c83-40a3-84a3-206ccf33329e","correlationId":"d4fdc6d7-4c83-40a3-84a3-206ccf33329e","projectId":"project_xw6uqnat7pomsfxh6iw","statusCode":401,"durationSeconds":0.0009999275207519531}
{"level":30,"time":1785924737102,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"d4fdc6d7-4c83-40a3-84a3-206ccf33329e","res":{"statusCode":401},"responseTime":0.5948339998722076,"msg":"request completed"}
{"level":30,"time":1785924737189,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"e273e75a-9055-40b0-8aad-3ff6ddda2fa4","event":"request.completed","requestId":"e273e75a-9055-40b0-8aad-3ff6ddda2fa4","correlationId":"e273e75a-9055-40b0-8aad-3ff6ddda2fa4","projectId":"project_5o0zlz0v0v6msfxh6la","statusCode":401,"durationSeconds":0.0009999275207519531}
{"level":30,"time":1785924737189,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"e273e75a-9055-40b0-8aad-3ff6ddda2fa4","res":{"statusCode":401},"responseTime":0.810666024684906,"msg":"request completed"}
{"level":30,"time":1785924737271,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"934a1944-0054-43c4-9d84-631261717609","event":"request.completed","requestId":"934a1944-0054-43c4-9d84-631261717609","correlationId":"934a1944-0054-43c4-9d84-631261717609","projectId":"project_eemstd42jycmsfxh6nm","statusCode":401,"durationSeconds":0.0009999275207519531}
{"level":30,"time":1785924737271,"pid":19035,"hostname":"MacBook-Pro-de-HB.local","reqId":"934a1944-0054-43c4-9d84-631261717609","res":{"statusCode":401},"responseTime":0.30491599440574646,"msg":"request completed"}
 ✓ src/tests/object-storage-token-auth.spec.ts (4 tests) 828ms
   ✓ object-storage routes — workspace token auth > authorizes a workspace token scoped to the project (no user session)  593ms

 Test Files  4 passed (4)
      Tests  38 passed (38)
   Start at  13:12:13
   Duration  3.87s (transform 1.62s, setup 0ms, collect 5.62s, tests 1.49s, environment 0ms, prepare 458ms)
