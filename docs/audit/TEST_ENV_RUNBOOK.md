...
> ### ⚠️ CORRECTION 2026-08-27 — le nom de release ne protège de RIEN
>
> Contrairement à ce qu'indiquait ce runbook, la release réellement déployée sur
> le cluster d'audit s'appelle **`vibecore`**, dans le namespace **`vibecore`** —
> **exactement comme la production** (audit : révision 20 ; prod : révision 1085).
>
> Le **seul** discriminant entre les deux est le **contexte Kubernetes**. Le
> contexte ambiant de la machine peut très bien être celui de la PROD (constaté).
> Donc :
>
> * **toujours** passer `--kube-context` / `--context` explicitement, dans
>   *chaque* commande `helm` et `kubectl` ;
> * ne jamais se fier au nom `vibecore-audit` — il ne correspond à aucune release
>   existante ; l'utiliser créerait une **seconde** release divergente ;
> * vérifier avant toute écriture :
>
> ```bash
> AUDIT_CTX=gke_vibecore-audit-test-20260807_europe-west9-a_vibecore-audit-cluster
> helm --kube-context "$AUDIT_CTX" list -n vibecore   # doit montrer la révision d'audit
> ```
>
> La commande d'upgrade correcte pour cet environnement est donc `helm upgrade
> vibecore … --kube-context "$AUDIT_CTX"`, et non `vibecore-audit`.
...
### 4.1 Deux pièges de build sur cet environnement (relevés le 2026-08-27)

**a) L'upload du contexte est le vrai goulot — le contourner.**
`git archive origin/main` produit un contexte de **358 Mo**, dont **264 Mo de `docs/`**
(`docs/parity` 163 Mo, `docs/deploy-evidence` 94 Mo) qu'aucun Dockerfile ne lit : le
`.gcloudignore` ignore `*.md` puis **ré-inclut** `!docs/**/*.md`, et les binaires de
`docs/` ne sont couverts par aucune règle. Débit mesuré vers GCS depuis un poste
domestique : **< 50 Ko/s** (20 Mo non téléversés en 6 min 40) — 342 Mo abandonnés
après 38 min.

Le repo étant **public**, la parade est de ne rien téléverser et de faire cloner le
SHA par Cloud Build lui-même :

```bash
# wrapper = cloudbuild.yaml avec une étape `clone` en tête, et build-deps
# qui attend ['prime-cache', 'clone']
gcloud builds submit --no-source \
  --project=vibecore-audit-test-20260807 --region=europe-west9 \
  --config=cloudbuild-clone.yaml \
  --substitutions="_PROJECT=vibecore-audit-test-20260807,_REPO=vibecore-audit-containers,_SHORT_SHA=<court>"
```
...
git remote add origin https://github.com/openaxcloud/vibecore.git
git fetch -q --depth 1 origin <SHA_COMPLET_40_CARACTERES>
git checkout -q FETCH_HEAD
```

⚠️ `git fetch` par SHA exige le **SHA complet (40 caractères)** — un SHA court donne
`fatal: couldn't find remote ref`. Avec cette méthode le build démarre
**immédiatement**, sans upload.

**b) Le fan-out à 7 images tue le worker sur CE projet (cache froid).**
`cloudbuild.yaml` lance les 7 builds de service **en parallèle** après `build-deps`.
En prod cela passe (**39 SUCCESS / 40 builds, aucun `INTERNAL_ERROR`**) parce que les
tags `:latest` sont chauds et que `--cache-from` évite l'essentiel du travail. Sur le
registry d'audit, les `:latest` datent du 13/08 : les 7 étapes exécutent alors chacune
un `pnpm deploy` complet (~2 100 paquets résolus) **simultanément** sur un
`E2_HIGHCPU_8` (8 Gio) — exactement le scénario d'OOM que documente l'en-tête de
`cloudbuild.yaml`. Résultat observé **deux fois** : toutes les étapes basculent en
`INTERNAL_ERROR`, log coupé net, **aucune image poussée**.

Parade : ne construire que les images nécessaires, **en série** (`waitFor` chaîné).
Pour une vérification front-end, `deps` → `web` → `api` suffit. Le reste de la
plateforme peut rester sur les images précédentes — en notant l'écart de version
comme réserve dans les preuves.

⚠️ **En élaguant `cloudbuild.yaml`, ne pas oublier le bloc `images:` en fin de
fichier.** C'est lui — et non les `--tag` des étapes `docker build` — qui
déclenche le **push** vers Artifact Registry. Un fichier tronqué aux seules
étapes `steps:` produit un build **`SUCCESS` qui ne pousse rien** : les images
sont construites dans le worker puis jetées avec lui. Symptôme exact : build vert,
et `gcloud artifacts docker images list … --sort-by=~CREATE_TIME` ne montre
toujours que les anciennes images. Garder donc :

```yaml
images:
  - ${_REGION}-docker.pkg.dev/${_PROJECT}/${_REPO}/deps:${_SHORT_SHA}
  - ${_REGION}-docker.pkg.dev/${_PROJECT}/${_REPO}/deps:latest
  - ${_REGION}-docker.pkg.dev/${_PROJECT}/${_REPO}/web:${_SHORT_SHA}
  - ${_REGION}-docker.pkg.dev/${_PROJECT}/${_REPO}/web:latest
  - ${_REGION}-docker.pkg.dev/${_PROJECT}/${_REPO}/api:${_SHORT_SHA}
  - ${_REGION}-docker.pkg.dev/${_PROJECT}/${_REPO}/api:latest
```

**Toujours vérifier le push, pas seulement le statut du build** — un `SUCCESS`
ne prouve pas qu'une image est arrivée :

```bash
gcloud artifacts docker tags list \
  europe-west9-docker.pkg.dev/vibecore-audit-test-20260807/vibecore-audit-containers/web \
  --project=vibecore-audit-test-20260807 --format='value(tag)' | grep <SHORT_SHA>
```

### 4.2 ⚠️ L'autoscaler de l'API épuise les connexions Postgres (relevé le 2026-08-27)

**Symptôme** : panneaux de l'IDE qui rendent par intermittence un état vide, `500`
sur des routes quelconques, et mesures QA non reproductibles d'un essai à l'autre.

**Cause** : le HPA de `api` est réglé **min 5 / max 25** sur cet environnement,
alors que le Cloud SQL est un **`db-g1-small`** (≈ 50 connexions au total, aucun
flag `max_connections` personnalisé). À 25 pods × pool Prisma, la limite est
dépassée mécaniquement. Les logs API affichent alors :
...
Relevé : **60 erreurs de ce type en 5 minutes** avec le HPA à 25 réplicas.

> **Ce n'est PAS un défaut de production.** En prod le même HPA est réglé
> **min 2 / max 6** contre un `db-custom-2-8192` : vérifié le 27/08,
> `kubectl -n vibecore get hpa vibecore-vibecore-platform-api` → `2 / 6 / 4`.
> L'écart est propre au dimensionnement de l'environnement de test.

**Parade avant toute campagne de mesure** — plafonner le HPA, sinon les résultats
sont inexploitables :

```bash
AUDIT_CTX=gke_vibecore-audit-test-20260807_europe-west9-a_vibecore-audit-cluster
kubectl --context "$AUDIT_CTX" -n vibecore patch hpa vibecore-vibecore-platform-api \
  --type=merge -p '{"spec":{"minReplicas":2,"maxReplicas":3}}'
```

⚠️ Un simple `kubectl scale` **ne suffit pas** : le HPA remonte les réplicas
quelques instants plus tard (constaté — passage de 2 à 10 puis 25 pods). Il faut
patcher le HPA lui-même.

**Vérifier que l'environnement est calme avant de mesurer** :

```bash
kubectl --context "$AUDIT_CTX" -n vibecore logs -l app.kubernetes.io/component=api \
  --tail=400 --since=2m | grep -c 'Too many database connections'   # doit rendre 0
```

### 4.3 Limite de débit sur `/auth/login` — reconnecter à chaque contexte casse les campagnes

`POST /auth/login` est plafonné à **10 tentatives par minute** (`AUTH_LOGIN_RATE_LIMIT_MAX`,
défaut 10, `services/api/src/app.ts`). Un harnais Playwright qui ouvre un contexte
par viewport et se reconnecte à chaque fois épuise ce quota au bout de quelques
essais : la connexion échoue silencieusement et le script casse sur un
`waitForFunction: Timeout` en attendant de quitter `/login` — ce qui **ressemble à
une panne de l'environnement alors que c'est le quota**.

Parade : se connecter **une seule fois**, sauvegarder l'état, puis le réutiliser
pour tous les contextes :

```js
await ctx.storageState({ path: 'state.json' });                 // une fois
await browser.newContext({ storageState: 'state.json', ... });  // ensuite
```
