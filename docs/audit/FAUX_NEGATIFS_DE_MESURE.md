...
| 29 | comptage des modules d'un graphe de build | j'ai compté des **identifiants de module** et lu le résultat comme un **poids**. Le graphe de Rollup est une structure de CONSTRUCTION ; il ne dit rien du coût de CHARGEMENT. Les 191 modules de route du chunk sont des coquilles vides : le framework retire les exports serveur (`loader`, `action`), il ne reste qu'un enregistrement qui ne rend presque aucun code | **« 223 modules de route dans la fermeture statique »** — un chiffre exact, une topologie réelle, et une demi-journée passée à vouloir détacher **10,3 Kio compressés**, soit 1,0 % du poids JS de la page | mesurer un poids **en octets**, sur l'artefact SERVI : `curl -w '%{size_download}'` sur les assets réellement préchargés dans le HTML. Trente secondes, contre deux tentatives de découpage et une régression |
| 30 | `manualChunks` de Rollup | poser une règle, c'est croire qu'on pose une **exception locale**. En réalité **on ne retire pas un module d'un groupe : on redéfinit le groupe**, et l'outil recalcule la partition ENTIÈRE, fusionnant ailleurs ce qu'il séparait avant | épingler un module : 223 → 221 routes (gain nul). Épingler une famille : 223 → **259** routes, avec **28 → 22 chunks**. Le signe qui alerte est là : **le nombre de chunks BAISSE quand on croyait en ajouter un** | ne jamais valider une règle sur elle-même, mais sur la **composition mesurée de tout le bundle**, avant et après. Rétroactivement, cela explique la page blanche de CodeMirror d'août mieux que l'explication d'alors : toute règle rebat la partition, donc toute règle peut créer un cycle **loin de l'endroit qu'on croyait toucher** |
| 31 | vérification dont on ne lit pas la valeur | j'avais posé la BONNE commande de contrôle — `git ls-remote` après un push — et je ne l'ai pas comparée à ce que j'attendais | **quarante caractères hexadécimaux en face du bon nom de branche.** La forme d'un succès. C'était le SHA du commit **précédent** : le `git add` avait échoué (chemin dans `.gitignore`), l'erreur masquée par un `2>/dev/null` | **une vérification n'en est une que si l'on COMPARE sa valeur à une valeur attendue.** Le silence alerte ; la forme rassure. **ET comparer deux valeurs ne suffit pas si l'on n'a pas vérifié qu'au moins une a BOUGÉ** — le 2026-09-05 ce même contrôle a validé un commit VIDE : `git add` avait échoué sur un répertoire absent, `HEAD` valait encore `main`, le distant aussi, et l'égalité rendait « identiques ». La leçon attrapée par elle-même. En pratique, deux conditions : `HEAD != base_attendue` **et** `HEAD == ls-remote` |
| 32 | message de commit entre guillemets doubles | des accents inverses y ont été vus par le shell comme une **substitution de commande** : le mot a été exécuté, puis remplacé par sa sortie vide | **la commande a RÉUSSI et le résultat était FAUX.** Ni zéro résultat, ni silence : un **succès partiel** dont la partie fausse n'est couverte par aucun contrôle — un mot mangé au milieu d'une phrase, dans le seul artefact que personne ne relit | écrire tout message non trivial dans un fichier (`git commit -F`) avec un heredoc **quoté** (`<<'EOF'`), qui supprime toute interprétation ; et pour un message déjà écrit, un `grep` qui **compare** à un mot attendu |
| 33 | une anomalie assortie d'une explication | ma propre ventilation affichait **2 requêtes « API » sur 73** pour ce qui était censé être une page projet. J'avais l'explication toute prête — « ma mesure s'arrête à `load` » — et je l'ai **adoptée au lieu de la vérifier** | la page mesurée rendait **404**. Sept « vagues » décrivant le chargement d'une page d'erreur, publiées comme la décomposition d'une page projet. La vraie page fait 132 requêtes, dont 8 d'état et 2 archives — le double | **une anomalie qu'on justifie n'est plus une anomalie, elle devient une note de bas de page.** Quand une explication vient trop vite, elle doit être VÉRIFIÉE, pas adoptée. Vérifier celle-ci coûtait une ligne : lire le statut HTTP du document |
| 34 | recherche d'un statut HTTP par sa chaîne dans un journal | `grep -c '425'` compte aussi les latences (`responseTime`), les identifiants et les horodatages qui contiennent ces trois chiffres. Mesuré le 2026-09-05 sur 6 h de journaux de l'API de production : **418 lignes contenaient « 425 », 3 seulement étaient une réponse 425** — un facteur 139 | un taux de refus catastrophique là où il n'y avait qu'un incident d'une seconde. Symétriquement, `grep -c '429'` rendait 1 594 pour **797** réponses réelles : chaque requête journalise DEUX lignes (`request.completed` puis `res`), donc tout comptage par chaîne est doublé | compter la forme structurée — `"res":{"statusCode":NNN}` — et non le nombre nu. **Témoin obligatoire sur des statuts voisins** (`401`, `404`, `429`) : s'ils rendent tous des comptes plausibles et distincts, la forme discrimine ; s'ils rendent tous le même ordre de grandeur, elle compte autre chose |
| 35 | silence d'un service dans ses journaux | j'ai conclu « le `workspace-manager` n'est jamais sollicité » de **une ligne de journal en 7 jours 13 heures**. Faux : il est construit avec `Fastify({ logger: false })` et ne journalise **aucune** requête — seulement des événements applicatifs. Son silence ne mesurait rien | une panne d'aiguillage inventée de toutes pièces, sur un service qui répondait parfaitement — sonde ultérieure : `/health` 200, sans jeton 401, avec jeton 200, et `POST /workspaces/start` créant un pod `1/1 Running` en 41 s | avant de lire un silence, **vérifier que le composant journalise** ce qu'on cherche : lire sa configuration de logger, ou faire apparaître une ligne connue. Un service muet par construction rend le même silence qu'un service jamais appelé. Sonder le COMPORTEMENT, jamais l'absence de trace |
| 36 | **garde-fou dont on n'a pas vérifié qu'il rougit** | j'allais ancrer le contrôle du mode d'exécution sur la présence de la chaîne `remote-kubernetes` dans le bundle. Mesuré : elle apparaît **6 fois dans le bundle CASSÉ** (comparaisons de type qui survivent à la minification) contre 10 dans le sain. Le contrôle serait passé au vert sur l'image défectueuse | un garde-fou d'apparence rigoureuse — il lit le bundle produit, il compte des occurrences, il a un message d'erreur — et qui n'attrape jamais le cas pour lequel il existe. **Un garde-fou dont on ne vérifie pas qu'il rougit sur le cas qu'il doit attraper n'est pas un garde-fou, c'est une décoration.** | compter les occurrences dans les DEUX artefacts avant de choisir l'ancre. Ici le discriminant était le repli constant-foldé `return"webcontainer"` : 1 dans le cassé, 0 dans le sain. Puis casser le garde et vérifier qu'il rougit — mesuré, 2 cas sur 5 tombent |
...
| 36 | une explication construite À DEUX | le pair avait la moitié serveur (« aucun mécanisme ne lance de serveur, tout le déclenchement est côté client »), j'avais la moitié client (« sur mobile l'auto-démarrage est désactivé »). Chaque moitié était VRAIE. Assemblées, elles donnaient une explication cohérente et fausse : « sur mobile, Avi n'a aucun moyen de lancer son application » | trois échanges à approfondir la branche mobile. **Une seule expérience — le MÊME projet sur les deux formats — l'a écartée en trois minutes** : desktop aussi émet zéro commande de démarrage. L'écart mobile/desktop n'existait pas | **quand une explication repose sur une CONDITION, mesurer aussi le cas où la condition est FAUSSE.** Une explication bâtie à deux, dont chaque moitié est vraie, se défend beaucoup mieux qu'une erreur solitaire — et aucune des règles précédentes ne l'attrape. Le contrôle manquant n'est pas d'approfondir la branche qu'on croit tenir, c'est de tester l'autre |
| 37 | compter des échecs sans compter les réussites | j'ai vu 5 écritures `ide-state` refusées en `412`, toutes portant le fil de conversation, et j'en ai tiré « le seul écrivain qui porte le fil est systématiquement refusé » | **une perte de données annoncée**, alors que la mesure complète disait : 18 écritures RÉUSSIES sur 29 portent le fil, compteur de messages à 10 des deux côtés. Un RETARD, pas une perte. Un dénominateur manquant transforme du bruit ordinaire en catastrophe | **toujours rendre le dénominateur.** « 5 refus » n'est pas une information ; « 5 refus sur 24 » en est une. C'est le pendant exact de la règle sur les faux négatifs, et on n'en avait que la moitié : on savait se méfier d'un zéro, pas d'un numérateur seul |
| 38 | un statut global qui ne distingue pas l'essentiel de l'accessoire | `gcloud builds submit` a rendu **`FAILURE`** alors que l'image était **construite ET poussée** : seule l'étape de signature avait échoué, l'API KMS étant désactivée sur le projet d'audit | j'ai failli reconstruire pour rien. Pire cas symétrique : croire qu'on n'a rien livré alors que si — ou l'inverse, croire qu'on a livré parce qu'une étape verte masque une étape rouge | lire le statut **PAR ÉTAPE**, jamais le verdict global d'un pipeline à étapes. `gcloud builds log <id> \| grep 'Step #'` répond en une commande, et le digest poussé se vérifie dans le registre indépendamment du statut |
...
## La règle que ces trente-sept cas imposent
