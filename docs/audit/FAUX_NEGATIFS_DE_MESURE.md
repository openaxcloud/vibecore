...
| 40 | une étiquette d'erreur générique posée sur un code de statut | la route web traduit **n'importe quel** `429` venu de l'amont en `PANEL_QUOTA_EXCEEDED`, dont le message dit « libérez des ressources ou passez à une offre supérieure ». La cause réelle était un compteur `snapshots.count` **sans fenêtre**, qui verrouille à vie au-delà de 5 | **trois sessions orientées vers le libellé plutôt que vers la cause.** J'ai d'abord lu « limitation de débit » (faux : la route ne passe par aucun limiteur), puis « quota qui refuse la première création » (faux : le projet était neuf, c'est l'organisation qui avait dépassé, ailleurs, il y a longtemps) | **une étiquette n'est pas un diagnostic.** Quand un code d'erreur est posé par une couche de traduction, il décrit ce que CETTE couche a compris, pas ce qui s'est produit en amont. Remonter au corps de la réponse d'origine, ou au compteur lui-même — jamais s'arrêter au libellé, surtout quand il propose une action commerciale |
...
## La règle que ces quarante cas imposent
