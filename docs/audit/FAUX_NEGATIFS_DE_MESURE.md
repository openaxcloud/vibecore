...
| 34 | l'enquête elle-même | j'ai cherché pourquoi `ide-state` partait sept fois, et testé trois hypothèses — clés de portée divergentes, module dupliqué dans deux chunks, boucle de réessai — **chacune réfutée par une mesure**. La vraie réponse n'était dans aucune : l'environnement servait du code du **1er septembre**, antérieur de quatre jours au correctif qui règle exactement ce défaut | trois diagnostics successifs sur un comportement **déjà corrigé**, dont un relayé comme un fait. Remesuré sur le bon code : `ide-state` = **1 GET** | **valider le TERRAIN avant d'ouvrir l'enquête, jamais l'inverse.** La quatrième hypothèse coûtait UNE commande — `git merge-base --is-ancestor <correctif> <sha servi>` — et devait être la première. Ce n'est pas un oubli de vérifier un SHA : **une hypothèse absorbe l'attention et fait sauter les vérifications de contexte**, précisément parce qu'on a une piste. La règle avait été appliquée partout ailleurs le même jour |
| 35 | un résultat trop propre | huit motifs de recherche rendant zéro d'affilée, avec un témoin à un seul. Cause : la sonde interceptait `window.fetch` alors que rien ne passait par là — instrument aveugle, pas absence de phénomène | **huit zéros alignés**, qui se lisaient comme « ce code ne fait rien de tout ça » | **un résultat trop propre est un signal au même titre qu'un résultat vide.** Une série parfaitement homogène — que des zéros, que des succès, que des valeurs identiques — doit faire regarder l'instrument avant la conclusion |
