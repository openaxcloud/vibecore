...
## 🔎 Passe SECURITE & DONNEES — 2026-09-04 (3e tour)

La file nommee etait **deja couverte** par les PR des deux tours precedents
(#398 chiffrement, #399 audit, #418 adversarial, #419/#403/#414 stockage,
#416 import, #420 roles DB). Ce tour porte donc sur les **deux points mesures
depuis**.

| Point | Resultat |
|---|---|
| Ressources de projet qui survivent a sa suppression | **AUDX-171 → PR #425** |
| `/project/<n'importe quoi>` a 200 sans authentification | **AUDX-172 → deja couvert par #408**, rien ajoute |

### La lecon du tour : une methode morte n'est pas un correctif

Le vrai defaut d'AUDX-171 n'etait pas un demontage **incomplet**, c'etait un
demontage **jamais appele**. `teardown()` existait, promettait dans son propre
commentaire d'empecher exactement les orphelines constatees, et n'etait invoque
que par des specs. **Une methode dont les seuls appelants sont ses tests est une
intention, pas un mecanisme** — et ses tests passent au vert pendant que la
production accumule les orphelines.

⚠️ **Corollaire pour la PR #396**, qui corrige un trou *a l'interieur* de
`teardown()` : elle etait necessaire et insuffisante, et rien dans son perimetre
ne pouvait le reveler. Verifier le **site d'appel** avant de corriger le corps.

### Et une retention ne survit pas a son proprietaire

`spec.backup.retentionPolicy` semblait etre la cause de la survie des
sauvegardes. C'est l'inverse : elle est appliquee **par l'operateur a travers le
`Cluster`**, donc supprimer le `Cluster` **gele** l'elagage au lieu de le
declencher. Une politique de conservation portee par la ressource cesse d'exister
avec elle.
...
| AUDX-171 | **Supprimer un projet ne supprimait AUCUNE de ses ressources externes** — `teardown()` existait et n'etait appele par personne | **P1** | **✅ CORRIGE — PR #425 (ouverte)** | Parti de **4 bases orphelines nettoyees a la main** (03/09). Les **lignes** sont propres : 24 des 25 relations vers `Project` sont en `onDelete: Cascade` (la 25e, `AiConversation`, est un `SetNull` delibere). Le defaut est ailleurs : **`DatabaseProvisioner.teardown()` est declare (l.483), implemente deux fois (l.516, l.700) et invoque UNIQUEMENT par des specs** — verifie par recherche exhaustive. Son commentaire promet pourtant « *so deleting a project leaves no orphaned production database behind* ». La route `DELETE /projects/:id/permanent` faisait `hardDeleteProject` + un audit, point final. **Survivaient : le `Cluster` CNPG dev ET prod (qui continue de tourner), la CR `Database` du tier partage, le `ScheduledBackup`, le seau `vc-<projectId>` (`deleteBucket` n'etait appele que par une route manuelle), et le PVC `pvc-<org>-<slug>` (supprime nulle part).** ⇒ **trois familles de ressources, pas une** : la reponse a « est-ce plus large qu'un seul correctif » est oui. ⚠️ **La politique de conservation aggrave le cas au lieu de le regler** : `spec.backup.retentionPolicy` est appliquee par l'operateur CNPG **a travers le `Cluster`** ; supprimer le `Cluster` ne libere pas les sauvegardes, il **gele leur elagage** — elles restent indefiniment. Une retention ne protege que tant que la ressource vit, elle ne survit pas a son proprietaire. **Correctif** : inventaire `project-teardown.ts` (greppable, en un endroit), demontage AVANT la suppression de la ligne (le nom du PVC vit sur cette ligne), echecs **captures et rendus** au lieu du `.catch(() => {})` qui rendait un demontage rate indiscernable d'un reussi. **Reste ❌ declare** : les sauvegardes barman `gs://<bucket>/db/<projectId>` — l'api n'a aucun acces en suppression a ce seau ; a traiter par une regle de cycle de vie. Inscrites dans `KNOWN_UNCOVERED_PROJECT_RESOURCES` plutot que dans l'inventaire avec un `remove` vide, qui aurait rapporte `removed: true` sur une ressource vivante. **Compose avec #396**, qui corrige un trou A L'INTERIEUR de `teardown()` : sans #396 le demontage rate le tier partage, sans #425 il n'a jamais lieu. | BE / RT | — |
| AUDX-172 | **`/project/<n'importe quoi>` servait une brochure publique a 200, sans authentification** | P1 | **✅ COUVERT — PR #408 (ouverte), pas par moi** | Signale le 04/09 ; **deja traite** avant que je l'ouvre. #408 remplace la brochure par un **301 vers la page canonique** `/projects/:id`, avec un garde anti-**open-redirect**. #423 couvre les deux jumelles `/editor/:id` et `/projects/:id/preview`. **Rien ajoute de mon cote** : dupliquer aurait cree un conflit sur les memes fichiers. ⚠️ **Classe a retenir** : la route `/profile/:username` avait ete corrigee pour ce defaut exact — son propre commentaire decrit « *fake data on a public URL* » — et les jumelles avaient ete oubliees. Un correctif de classe doit enumerer les FABRIQUES (`create*SurfacePage` : 8) et non la seule route signalee. | FE | — |
