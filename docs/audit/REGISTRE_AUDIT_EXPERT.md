...
| AUDX-018 | **Réservation atomique de crédits AVANT** appel fournisseur | P0 | NON_COMMENCÉ | **Non entamé, et non maquillé.** C'est le correctif *structurel* du metering : tant que le débit dépend d'un rapport **postérieur** à l'appel, un rapport absent ou minoré = IA gratuite — AUDX-017 rend cela **visible**, pas **impossible**. Exige réserver → appeler → solder, avec compensation sur échec. Chantier distinct. | BILL | — |
...
| `✅ CORRIGÉ` (PR ouverte, non mergée) | 3 | **AUDX-003** — PR #354 ; **AUDX-001** — PR #355 ; **AUDX-008** — PR #363. |
| `🟠 PARTIEL` (PR ouverte, reste identifié) | 4 | **AUDX-004** — PR #357 : usage unique non livré, exige un magasin `jti`. **AUDX-005** — PR #361 : code prêt, bascule d'exploitation non faite (D-02 encore non tenue). **AUDX-006** — PR #362 : screenshotter câblé, Git et webhook non câblés. **AUDX-017** — PR #365 : provenance tracée, sous-déclaration visible mais pas impossible. |
