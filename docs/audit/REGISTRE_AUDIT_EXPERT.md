...
| AUDX-007 | #363 + **#373** | 🟠 **1 des 5 migré** | **#373 fait Vercel de bout en bout** comme migration de référence : montée du jeton au serveur puis effacement local **uniquement sur succès confirmé** (l'inverse serait une perte de données), et arrêt de la persistance du jeton. Helper rangé dans `app/lib/connections/` — **pas** dans le composant — pour que les 4 suivants soient des copies (AUDX-163). **Reste** : `github`, `gitlab`, `netlify`, `supabase`, plus le **routage des appels** par `connector-proxy`, non entamé. |
...
## 12-bis. Points constatés HORS REMISE — AUDX-159 → AUDX-168
...
| AUDX-167 | **Une LENTEUR devenue PANNE — `GET /api/projects/<id>/ide-state` à 572–3 914 ms** | **P0** | **💻 Livré (partiel) — PR #374** | **Mesuré en production** : 572 à **3 914 ms**, **tout côté serveur avant le premier octet**, contre **220 ms** pour `/api/health` sur la même infrastructure. Franchit le **seuil d'abandon de 5 s** du client ⇒ **panneau de conversation vide pour toute la durée de la page**. ⚠️ **La lenteur ne dégrade pas la fonction, elle la SUPPRIME** — d'où une ligne de registre et non un ticket de performance. **Deux faits établis dans le code** (distincts de la mesure) : (a) la route **émettait un ETag et ignorait `If-None-Match`**, donc chaque chargement resérialisait tout ; (b) le blob est **NON BORNÉ** — le `PUT` accepte jusqu'à `API_BODY_LIMIT_BYTES`, **25 Mo**. **Fait (#374)** : GET conditionnel, une revisite coûte un **304 sans corps**. ⚠️ **Ne rend PAS le premier chargement plus rapide** ; supprime le coût **répété**. ⚠️ **Production non profilée** : attribuer les 3 914 ms à une cause unique serait une déduction — le blob non borné reste **une piste à mesurer**. ⚠️ Bornage du blob au `PUT` **volontairement pas fait** : casserait la sauvegarde des états déjà volumineux (règle 19). **Lien** : même symptôme qu'**AUDX-164** (panneau vide) pour une **cause indépendante** — illustration directe d'**AUDX-165**. 🔴 **CAUSE RÉELLE ÉTABLIE LE 02/09 — CE N'EST PAS CETTE ROUTE.** Voir **AUDX-168** : les pods API sont **étranglés par leur limite CPU 92 % du temps** ; **toute** requête authentifiée est lente. Base de données à **3 ms**, Redis à **1,4 ms** — le stockage est hors de cause. ⚠️ **Ce que cela valide, et qu'il faut retenir** : j'avais refusé d'attribuer les 3 914 ms à une cause unique faute d'avoir profilé la production. Ce refus a **évité une fausse conclusion** — et, plus concrètement, a évité de « corriger » une route saine et de compter le point comme fermé pendant que la vraie cause continuait d'agir. **C'est exactement AUDX-165 appliqué à une mesure de performance.** Le GET conditionnel de #374 **reste utile** (un cache à moitié câblé ne servait à rien) mais **ne traite pas la cause** ; le point ne se ferme pas dessus. | BE / FE |
| AUDX-168 | **Les pods API sont étranglés par leur limite CPU 92 % du temps** — toute requête authentifiée est lente | **P0** | NON_COMMENCÉ | **Mesuré en production le 02/09** : **8 341 secondes d'attente CPU cumulée**, throttling **92 % du temps**. En regard : base de données **3 ms**, Redis **1,4 ms** — le stockage n'est pour rien dans la latence. ⚠️ **C'est la cause commune** derrière AUDX-167 (ide-state à 3 914 ms) et vraisemblablement derrière plusieurs autres lenteurs attribuées à des routes individuelles. **Une seule cause en amont, des symptômes partout en aval — AUDX-165 à l'échelle de la plateforme.** ⚠️ **Conséquence de méthode** : toute optimisation de route entreprise avant de lever cet étranglement mesurera surtout le bruit du throttling, et risque de « corriger » du code sain. **Action** : relever la limite CPU (ou passer à une requête/limite dimensionnée sur le profil réel), puis **re-mesurer** ide-state avant de conclure quoi que ce soit sur la route. ⛔ Réglage d'infrastructure Helm/GKE — **hors PR**, relève d'Avi. | AVI / RT |
...
Contrôle d'exhaustivité : **158 lignes** couvrant la remise, aucune perte — plus **10 lignes hors remise** (§12-bis), qui prouvent que le registre n'est pas exhaustif du réel.
...
| Points constatés **hors remise** | AUDX-159 → 168 | 10 |
| **Total** | | **168** |
