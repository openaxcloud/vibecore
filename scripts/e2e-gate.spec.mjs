...
describe('bounded E2E waiver policy', () => {
  ...
  it('rejects non-boolean unstable flags that could bypass stale-waiver cleanup', () => {
    const result = runGate(
      {
        created: isoOffset(0),
        expires: isoOffset(7),
        waived: [{ ...waiver('sample.spec.ts › passes'), unstable: 'true' }],
      },
      report(),
    );
    expect(result.status).toBe(1);
    expect(result.stderr).toContain('unstable must be a boolean');
  });

  it('rejects duplicate waiver keys after path and position normalization', () => {
    const result = runGate(
      {
        created: isoOffset(0),
        expires: isoOffset(7),
        waived: [waiver('tests/e2e/sample.spec.ts:12:3 › fails'), waiver('sample.spec.ts › fails')],
      },
      report('unexpected', 'fails'),
    );
    expect(result.status).toBe(1);
    expect(result.stderr).toContain('duplicated after normalization');
  });

  it('rejects an empty report after the Playwright exit code was suppressed', () => {
    const result = runGate({ created: null, expires: null, waived: [] }, { suites: [] });
    expect(result.status).toBe(1);
    expect(result.stderr).toContain('contains no executable tests');
  });

  it('keeps a duplicate failure even when a later duplicate passes', () => {
    const failing = report('unexpected', 'duplicate');
    const passing = report('expected', 'duplicate');
    const result = runGate(
      { created: null, expires: null, waived: [] },
      { suites: [...failing.suites, ...passing.suites] },
    );
    expect(result.status).toBe(1);
    expect(result.stderr).toContain('NOT waived');
  });
