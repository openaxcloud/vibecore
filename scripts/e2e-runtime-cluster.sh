...
evidence() {
  ...
  local audited_sha="${GITHUB_SHA:-$(git rev-parse HEAD)}"
  local pods_json
  local storage_json
...
case "${1:-}" in
  ...
  assert-down)
    ...
    ;;
  *)
    echo 'usage: scripts/e2e-runtime-cluster.sh {up|assert-up|evidence [path]|purge-workspaces|down|assert-down}' >&2
    exit 2
    ;;
esac
