...
describe('SEC-9 — cutover detection reads the production bundle', () => {
  it('GREEN on the real repository: the interlock ships in the access authority, no test file in the graph', () => {
    const result = verifyInterlock(REPO_ROOT);

    expect(result.failures).toEqual([]);
    expect(result.ok).toBe(true);
    expect(result.attestation.carriers).toContain('services/api/src/deployment-access.ts');
