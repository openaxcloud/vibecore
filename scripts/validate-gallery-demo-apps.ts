...
const evidenceRoot = path.join(rootDirectory, 'docs', 'ui-ux-evidence', '2026-07-16', 'community-gallery', 'runtime');
const selectedApp = process.argv.find((argument) => argument.startsWith('--app='))?.split('=')[1];
const skipInstall = process.argv.includes('--skip-install');
const skipBuild = process.argv.includes('--skip-build');
const defaultPort = 43_100;
...
async function main() {
  ...
  const reportPath = path.join(evidenceRoot, 'gallery-demo-app-validation.json');
...
async function validateGalleryApp(app: GalleryDemoAppDefinition, port: number) {
  ...
  if (packageJson) {
    if (skipInstall) {
      ...
      await runCommand('pnpm', ['install', '--ignore-workspace', '--no-frozen-lockfile'], appDirectory, 12 * 60_000);
      install = 'passed';
    }
    ...
    if (packageJson.scripts?.build) {
      if (skipBuild) {
        ...
        await runCommand('pnpm', ['run', 'build'], appDirectory, 10 * 60_000, {
