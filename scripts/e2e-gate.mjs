...
if (waived.length > 0) {
  ...
  for (const [index, entry] of waived.entries()) {
    ...
    const normalizedTest = normaliseKey(entry.test);
    if (seen.has(normalizedTest)) fail(`${label}.test is duplicated after normalization: ${entry.test}`);
    seen.add(normalizedTest);
    ...
    if (entry.unstable !== undefined && typeof entry.unstable !== 'boolean') {
      fail(`${label}.unstable must be a boolean when present`);
    }
  }
}
  ...
  for (const suite of suites ?? []) {
    ...
    for (const spec of suite.specs ?? []) {
      ...
      if (!Array.isArray(spec.tests) || spec.tests.length === 0) {
        fail(`reported test has no executable result: ${key}`);
      }

      const ok = spec.tests.every((t) => t.status === 'expected' || t.status === 'skipped' || t.status === 'flaky');

      const wasFlaky = spec.tests.some((t) => t.status === 'flaky');

      /*
       * The same title can appear more than once in a malformed/multi-project
       * report. Aggregate fail-closed: a later passing duplicate must never
       * overwrite an earlier failure.
       */
      const previous = results.get(key);
      results.set(key, !ok || previous === 'failed' ? 'failed' : 'passed');
...
if (results.size === 0) {
  fail('Playwright report contains no executable tests');
}
