...
function realFiles() {
  return {
    ...
    dryRunWorkflow: fs.readFileSync(RELEASE_GATE_DRYRUN_WORKFLOW, 'utf8'),
...
describe('deploy gate wiring', () => {
  ...
  it('catches a movable third-party Action tag in the production path', () => {
    const files = realFiles();
    files.deployWorkflow = files.deployWorkflow.replace(/actions\/checkout@[0-9a-f]{40}/, 'actions/checkout@v4');
    expect(checkGateWiring(files).join('\n')).toMatch(/must be pinned to a full 40-hex commit/);
  });

  it('catches dispatch input interpolation into shell source', () => {
    const files = realFiles();
    files.deployWorkflow = files.deployWorkflow.replace(
      '          set -euo pipefail\n          EXPECTED=',
      '          set -euo pipefail\n          echo "${{ inputs.target_sha }}"\n          EXPECTED=',
    );
    expect(checkGateWiring(files).join('\n')).toMatch(/interpolated directly into run shell source/);
  });

  it('catches a validated staging output re-interpolated into shell source', () => {
    const files = realFiles();
    files.stagingWorkflow = files.stagingWorkflow.replace(
  ...
  it('requires strict protected-environment validation before WIF', () => {
    const files = realFiles();
    files.deployWorkflow = files.deployWorkflow.replace(
      'node scripts/validate-production-enterprise.mjs --strict --no-dotenv',
      'echo validation-disabled',
    );
    expect(checkGateWiring(files).join('\n')).toMatch(/strict production validation/);
  });
  ...
  it('catches break-glass losing its second approval gate', () => {
    const files = realFiles();
    files.breakGlassWorkflow = files.breakGlassWorkflow.replace(
      /production-break-glass-1/g,
      'production-break-glass-2',
    );
    expect(checkGateWiring(files).join('\n')).toMatch(/missing sequential approval environment/);
  });

  it('catches break-glass dispatched with an untrusted workflow graph', () => {
    const files = realFiles();
    files.breakGlassWorkflow = files.breakGlassWorkflow.replace(
      '.github/workflows/deploy-break-glass.yml@refs/heads/main',
      '.github/workflows/deploy-break-glass.yml@refs/heads/feature',
    );
    expect(checkGateWiring(files).join('\n')).toMatch(/pin github\.workflow_ref|graph pin/);
  });

  it('catches break-glass cosign installation without the pinned checksum', () => {
    const files = realFiles();
    files.breakGlassWorkflow = files.breakGlassWorkflow.replace(
      'echo "${COSIGN_SHA256}  /tmp/cosign" | sha256sum -c -',
      'echo checksum-disabled',
    );
    expect(checkGateWiring(files).join('\n')).toMatch(/cosign 2\.4\.3 must be checksum-verified/);
  });
  ...
  it('keeps staging from becoming a second production path', () => {
    ...
    expect(checkGateWiring(files).join('\n')).toMatch(/rejected when it equals production/);
  });

  it('catches staging dispatched with an untrusted workflow graph', () => {
    const files = realFiles();
    files.stagingWorkflow = files.stagingWorkflow.replace(
      '.github/workflows/deploy-staging.yml@refs/heads/main',
      '.github/workflows/deploy-staging.yml@refs/heads/feature',
    );
    expect(checkGateWiring(files).join('\n')).toMatch(/pin github\.workflow_ref/);
  });

  it('catches a staging tag validator weakened back to arbitrary tags', () => {
    const files = realFiles();
    files.stagingWorkflow = files.stagingWorkflow.replace("'^sha-[0-9a-f]{7}$'", "'^[^ ]+$'");
    expect(checkGateWiring(files).join('\n')).toMatch(/exactly sha-<7 lowercase hex>/);
  });

  it('catches staging Helm falling back from resolved digests to the tag', () => {
    const files = realFiles();
    files.stagingWorkflow = files.stagingWorkflow.replace(
      'services.${key}.imageDigest=${digest}',
      'services.${key}.imageTag=${IMAGE_TAG}',
    );
    expect(checkGateWiring(files).join('\n')).toMatch(/must consume resolved digests/);
  });

  it('catches staging accepting more than one GAR digest for a tag', () => {
    const files = realFiles();
    files.stagingWorkflow = files.stagingWorkflow.replace('"${#digests[@]}" -ne 1', '"${#digests[@]}" -lt 1');
    expect(checkGateWiring(files).join('\n')).toMatch(/exactly one valid sha256 digest/);
  });

  it('catches the gitleaks archive checksum being removed', () => {
    const files = realFiles();
    files.deployWorkflow = files.deployWorkflow.replace(
      'echo "${GITLEAKS_SHA256}  /tmp/gitleaks.tgz" | sha256sum -c -',
      'echo checksum-disabled',
    );
    expect(checkGateWiring(files).join('\n')).toMatch(/gitleaks 8\.21\.2 must be checksum-verified/);
  });
...
describe('service matrix must not drift from the chart or the rollout wait', () => {
  ...
  it('catches a service that would be waited on but never verified', () => {
    const files = realFiles();
    files.deployWorkflow = files.deployWorkflow.replace(
      'for svc in web admin api',
      'for svc in web admin screenshotter api',
    );
