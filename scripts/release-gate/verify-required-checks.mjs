...
export function validateWaiver(wf, nowMs = Date.now(), maxDays = WAIVER_MAX_DAYS) {
  ...
  if (expiry.getUTCFullYear() !== year || expiry.getUTCMonth() !== month - 1 || expiry.getUTCDate() !== day) {
    problems.push(
    ...
    requiredWorkflows: [
      {
        ...
        requiredJobs: ['Install, test, build, scan'],
      },
    ],
  };
}
  ...
  return [
    {
      ...
      expect: 'PASS',
      run: () =>
        evaluateRequiredChecks({ policy, targetSha: SHA_A, workflowRuns: [fixtureRun()], jobsByRunId: jobs() }),
    },
    {
    ...
    },
    {
      ...
      expect: 'REFUSE',
      run: () =>
        evaluateRequiredChecks({ policy, targetSha: 'a3f40f6a', workflowRuns: [fixtureRun()], jobsByRunId: jobs() }),
    },
  ];
}
  ...
  for (;;) {
    ...
    if (!args.wait || Date.now() >= deadline) {
      ...
      result = {
        ...result,
        verdict: 'REFUSE',
        refusals: [...result.refusals, ...result.waiting.map((w) => `${w} — still not green at the gate deadline`)],
      };
      break;
    }
    console.log(
