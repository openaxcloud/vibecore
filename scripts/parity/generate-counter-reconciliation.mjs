...
export function computeCounterReconciliation() {
  ...
  const lines = [
    ...
    row('P0 refusés (lot 57febeab)', refused.length, "count(reviewVerdict=REFUSED)", refused.join(', '), 'P0_REGISTRY.yaml'),
    row('P0 signés (lot 57febeab)', signed.length, "count(reviewVerdict=SIGNED)", signed.join(', '), 'P0_REGISTRY.yaml'),
    row('P0 OPEN', byStatus('OPEN').length, "count(status=OPEN)", byStatus('OPEN').join(', '), 'P0_REGISTRY.yaml'),
    row('P0 PROVEN_REVIEW_PENDING', byStatus('PROVEN_REVIEW_PENDING').length, "count(status=PROVEN_REVIEW_PENDING) = 22 signés (reçu incomplet) + 8 remédiés (re-signature attendue)", byStatus('PROVEN_REVIEW_PENDING').join(', '), 'P0_REGISTRY.yaml'),
    ...
    `   encore ouverts (${openNotRefused.join(', ')}) = **${byStatus('OPEN').length}**.`,
    '',
