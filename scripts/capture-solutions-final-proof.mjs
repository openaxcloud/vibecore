...
function applyLocalAuditIssues(result, audit, contentLanguage) {
  ...
  const expectedMarketingPage = slug === 'app-builder' ? 'app-builder' : `solution-${slug}`;
  const canonicalBase = `https://e-code.ai${expectedPath}`;
  const expectedCanonical = `${canonicalBase}?lang=${language}`;
  ...
  if (
    audit.document.mainMarketingPage !== expectedMarketingPage ||
    (slug !== 'app-builder' && audit.document.mainSolutionSlug !== slug)
  ) {
...
function eventCollectors(page, baseOrigin) {
  ...
  page.on('requestfailed', (request) => {
    ...
    const sameOrigin = request.url().startsWith(`${baseOrigin}/`);

    if (
      sameOrigin &&
      IMPORTANT_RESOURCE_TYPES.has(request.resourceType()) &&
  ...
  page.on('response', (response) => {
    ...
    const sameOrigin = response.url().startsWith(`${baseOrigin}/`);

    if (sameOrigin && IMPORTANT_RESOURCE_TYPES.has(request.resourceType()) && response.status() >= 400) {
      networkErrors.push({
        kind: 'http-error',
        resourceType: request.resourceType(),
        url: response.url(),
...
function applyEventIssues(result, events) {
  ...
  if (result.evidence.networkErrors.length > 0) {
    addIssue(
      ...
      `${result.evidence.networkErrors.length} same-origin critical resource error(s) occurred`,
...
function buildMarkdownReport(report) {
  ...
  const lines = [
    ...
    `- Scope: app-builder, website-builder, game-builder, dashboard-builder, chatbot-builder, internal-ai-builder, startups, freelancers (Enterprise excluded)`,
    '',
    '## Counts',
    '',
  ...
  lines.push(
    ...
    '## Row evidence',
    '',
    '| Page | Lang | Theme | Viewport | Result | Overflow | Clipped | Broken images | EN↔FR | Theme asset | Screenshot | Issues |',
    '| --- | --- | --- | ---: | --- | ---: | ---: | ---: | --- | --- | --- | --- |',
  );
  ...
  for (const row of rows) {
    ...
    lines.push(
      `| ${row.slug} | ${row.language} | ${row.theme} | ${row.viewport.width}×${row.viewport.height} | ${mark(
        row.status === 'passed',
      )} | ${row.evidence.overflow?.documentOverflowPx ?? 'n/a'} | ${row.evidence.clippedContent?.count ?? 'n/a'} | ${
