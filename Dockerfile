...
# Cap V8 heap so a runaway request can't OOM-kill the container.
# 512Mi limit (Helm) → leave ~120Mi for non-heap (workers, native, syscalls).
...
# Copy only what's needed to run the Node Remix server in production.
