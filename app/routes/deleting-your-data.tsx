...
} from '~/lib/i18n/catalogs/marketing-exact-deleting-your-data';
import { resolveRequestLocale } from '~/lib/i18n/request-locale';
import { MARKETING_SITE_URL, socialMetaTags } from '~/utils/social-meta';

const DELETING_DATA_ROUTES = {
...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  ...
  const seo = getMarketingExactDeletingYourDataCopy(language).exactDeletingYourData.seo;
  const canonical = `${MARKETING_SITE_URL}/deleting-your-data`;
...
export default function DeletingYourDataPage() {
  ...
  const copy = getMarketingExactDeletingYourDataCopy(language).exactDeletingYourData;

  const values = {
    count: formatMarketingExactDeletingYourDataInteger(DELETION_GRACE_PERIOD_DAYS, language),
  };

  const localizeText = (value: string) => interpolateMarketingExactDeletingYourDataCopy(value, values);

  return (
    <PublicShell>
      <main
        className="min-w-0 bg-[var(--ecode-background)] text-[var(--ecode-text)]"
        data-ecode-marketing-page="deleting-your-data"
        data-testid="page-deleting-your-data"
      >
        <section
          className="border-b border-[var(--ecode-border)] bg-gradient-to-b from-background to-muted"
          aria-labelledby="deleting-data-heading"
        >
          <div className="container-responsive py-16 sm:py-20 lg:py-24">
            <div className="mx-auto min-w-0 max-w-4xl text-center">
              ...
              <div className="mx-auto mt-8 flex max-w-3xl flex-col justify-center gap-3 sm:flex-row">
                <DataActionLink label={copy.actions.primary} to={DELETING_DATA_ROUTES.primary} />
                <DataActionLink
                  label={copy.actions.secondary}
                  to={DELETING_DATA_ROUTES.secondary}
                  variant="secondary"
                />
              </div>
            </div>
        ...
        <section
          ...
          aria-labelledby="deleting-data-highlights-heading"
          data-testid="section-deleting-data-highlights"
        >
          <div className="container-responsive max-w-6xl">
            <h2 id="deleting-data-highlights-heading" className="sr-only">
            ...
            <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {copy.highlights.items.map((highlight) => (
                <li
                  ...
                  data-testid={`highlight-deleting-data-${highlight.id}`}
                >
        ...
        <section
          ...
          aria-labelledby="deleting-data-guide-heading"
          data-testid="section-deleting-data-guide"
        >
          <div className="container-responsive max-w-6xl">
            <h2 id="deleting-data-guide-heading" className="sr-only">
              {copy.guide.title}
            </h2>
            <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
              {copy.guide.sections.map((section) => (
                <article
                  ...
                  aria-labelledby={`deleting-data-section-${section.id}`}
                  data-testid={`article-deleting-data-${section.id}`}
                >
                  <h3
                    id={`deleting-data-section-${section.id}`}
        ...
        <section className="py-14 sm:py-20" aria-labelledby="deleting-data-cta-heading">
          <div className="container-responsive max-w-6xl">
