...
export default function LandingPage() {
  return (
    <PublicShell>
      <section className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_520px] lg:py-20">
        <div className="flex min-h-[520px] flex-col justify-center">
          ...
          <p className="mt-5 max-w-2xl text-base leading-7 text-bolt-elements-textSecondary">
            VibeCore wraps the Bolt editor with persistent projects, remote runtimes, billing, quotas, audit controls
            and enterprise identity without removing the IDE engineers already use.
          </p>
        ...
        <div className="self-center rounded-lg border border-bolt-elements-borderColor bg-bolt-elements-background-depth-2 p-4 shadow-sm">
          <div className="grid h-[420px] grid-cols-[150px_1fr] overflow-hidden rounded-md border border-bolt-elements-borderColor bg-bolt-elements-background-depth-1">
            <aside className="border-r border-bolt-elements-borderColor p-3">
              {['app', 'components', 'services', 'packages'].map((item) => (
                <div key={item} className="mb-2 rounded px-2 py-1.5 text-sm text-bolt-elements-textSecondary">
                  {item}
                </div>
              ))}
            </aside>
            <div className="grid grid-rows-[44px_1fr_120px]">
