...
describe('E-Code public runtime API adapters', () => {
  it('serves public RAG, about and auth-user contracts without requiring a session', async () => {
    const ragStats = (await (await ragStatsLoader(loaderArgs('http://app.e-code.ai/api/rag/stats'))).json()) as {
      isAvailable: boolean;
      providers: Record<string, boolean>;
    };
    const about = (await (await aboutLoader(loaderArgs('http://app.e-code.ai/api/about'))).json()) as {
      values: unknown[];
      milestones: unknown[];
      team: unknown[];
    };

    const authUser = await (await authUserLoader(loaderArgs('http://app.e-code.ai/api/auth/user'))).json();
