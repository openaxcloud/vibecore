...
export const meta: MetaFunction = () => [
  { title: 'Agent features — VibeCore' },
  {
    name: 'description',
    content:
      'How the VibeCore IDE agent works: @ mentions, slash commands, plan-first checklist, per-hunk patch review, conversation branches, share links, presence and FR/EN i18n.',
  },
];
...
// eslint-disable-next-line @typescript-eslint/naming-convention -- Icon is destructured into a component-style alias for JSX rendering
function FeatureSection({ id, icon: Icon, title, tagline, why, steps, preview, notes }: FeatureSectionProps) {
  return (
    <section id={id} className="bolt-feature-doc-section">
      <div className="bolt-feature-doc-header">
        ...
        <div>
          ...
          <p className="text-sm text-bolt-elements-textSecondary">{tagline}</p>
        </div>
      </div>
      <div className="bolt-feature-doc-grid">
        <div className="bolt-feature-doc-copy">
          <p className="text-sm text-bolt-elements-textSecondary">{why}</p>
          ...
          {notes ? <div className="bolt-feature-doc-notes">{notes}</div> : null}
        </div>
        <figure className="bolt-feature-doc-preview" aria-label={`${title} preview`}>
          {preview}
          <figcaption className="bolt-feature-doc-caption">
            UI mockup — replace with a real screenshot in <code>/public/marketing/agent-{id}.png</code> once captured.
          </figcaption>
        </figure>
      </div>
    </section>
 ...
 * marketing page is meaningful even before someone uploads a real PNG.
 * They use the same SCSS tokens as the IDE so they pick up the active
 * theme.
 */
  ...
  return (
    <svg viewBox="0 0 360 230" role="img" aria-label="Plan checklist mockup" className="bolt-feature-doc-svg">
      ...
      <text x="20" y="50" fill="var(--vc-ide-text-secondary)" fontSize="11">
        2 / 4 complete · 1 failed
      </text>
      <rect x="20" y="58" width="320" height="4" rx="2" fill="var(--vc-ide-bg-overlay)" />
      <rect x="20" y="58" width="160" height="4" rx="2" fill="var(--vc-ide-accent-action, #3b82f6)" />
      {(
        [
          ['done', 'Read existing files', 'No conflicts detected', '#16a34a'],
          ['done', 'Generate the new module', 'src/feature.ts (43 lines)', '#16a34a'],
          ['running', 'Run unit tests', 'pnpm test --filter feature', 'var(--vc-ide-accent-action)'],
          ['failed', 'Deploy preview', 'Build failed at step 3', '#dc2626'],
        ] as const
      ).map(([status, desc, sub, color], idx) => (
        <g key={desc} transform={`translate(20 ${80 + idx * 32})`}>
          <circle cx="8" cy="10" r="6" fill={color as string} />
          <text
            x="24"
            y="14"
            fill="var(--vc-ide-text-primary)"
            fontSize="12"
            fontWeight={status === 'done' ? '400' : '500'}
            textDecoration={status === 'done' ? 'line-through' : 'none'}
          >
            {desc}
          </text>
          <text x="24" y="26" fill="var(--vc-ide-text-secondary)" fontSize="10">
            {sub}
          </text>
        </g>
      ))}
...
function PatchReviewPreview() {
  return (
    <svg viewBox="0 0 360 240" role="img" aria-label="Patch review mockup" className="bolt-feature-doc-svg">
      <rect x="0" y="0" width="360" height="240" rx="12" fill="var(--vc-ide-bg-elevated)" />
      <rect
        x="20"
        y="20"
        ...
        height="32"
        rx="6"
        fill="var(--vc-ide-bg-subtle)"
        stroke="var(--vc-ide-border-subtle)"
      />
      <text x="32" y="40" fill="var(--vc-ide-text-primary)" fontSize="13" fontWeight="600">
        Files changed
      </text>
      <text x="142" y="40" fill="var(--vc-ide-text-secondary)" fontSize="12">
        3 files
      </text>
      <text x="200" y="40" fill="#16a34a" fontSize="12" fontWeight="600">
        +24
      </text>
      <text x="232" y="40" fill="#dc2626" fontSize="12" fontWeight="600">
        −8
      </text>
      <rect x="248" y="26" width="84" height="22" rx="5" fill="var(--vc-ide-accent-action, #3b82f6)" />
      <text x="290" y="41" fill="white" fontSize="11" textAnchor="middle">
        Apply all (3)
      </text>
      <rect
        x="20"
        ...
        stroke="var(--vc-ide-border-subtle)"
      />
      <text x="32" y="84" fill="var(--vc-ide-text-primary)" fontSize="12" fontFamily="ui-monospace, monospace">
        src/feature.ts
      </text>
      <text x="180" y="84" fill="var(--vc-ide-text-secondary)" fontSize="11">
        @@ -1,3 +1,12 @@
      </text>
      <rect x="24" y="96" width="312" height="22" fill="rgba(220,38,38,0.12)" />
      <text x="32" y="112" fill="#dc2626" fontSize="11" fontFamily="ui-monospace, monospace">
        − export const value = 0;
      </text>
      <rect x="24" y="120" width="312" height="22" fill="rgba(22,163,74,0.12)" />
      <text x="32" y="136" fill="#16a34a" fontSize="11" fontFamily="ui-monospace, monospace">
        + export const value = computeValue();
      </text>
      <rect x="24" y="150" width="62" height="22" rx="4" stroke="var(--vc-ide-border-subtle)" fill="transparent" />
      <text x="55" y="165" fill="var(--vc-ide-text-primary)" fontSize="11" textAnchor="middle">
        Accept
      </text>
...
function ShareLinkPreview() {
  return (
    <svg viewBox="0 0 360 200" role="img" aria-label="Share link mockup" className="bolt-feature-doc-svg">
      <rect x="0" y="0" width="360" height="200" rx="12" fill="var(--vc-ide-bg-elevated)" />
      <text x="20" y="36" fill="var(--vc-ide-text-primary)" fontSize="15" fontWeight="600">
        Add a dark mode toggle
      </text>
      <text x="20" y="54" fill="var(--vc-ide-text-secondary)" fontSize="11">
        Shared from project demo-app on May 17 18:30
      </text>
      <rect
        x="20"
        y="72"
        ...
        stroke="var(--vc-ide-border-subtle)"
      />
      <text x="32" y="92" fill="var(--vc-ide-text-secondary)" fontSize="10" fontWeight="600" letterSpacing="0.04em">
        USER
      </text>
      <text x="32" y="112" fill="var(--vc-ide-text-primary)" fontSize="12">
      ...
      <rect
        ...
        stroke="var(--vc-ide-border-subtle)"
      />
      <text x="32" y="156" fill="var(--vc-ide-text-secondary)" fontSize="10" fontWeight="600" letterSpacing="0.04em">
        ASSISTANT
      </text>
      <text x="32" y="176" fill="var(--vc-ide-text-primary)" fontSize="12">
        Done — added ThemeSwitch.tsx and wired storage.
      </text>
    </svg>
  );
}
  ...
  return (
    <PublicShell>
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
        <header className="bolt-feature-doc-hero">
          <div className="bolt-feature-doc-hero-eyebrow">
            <Sparkles className="h-4 w-4" aria-hidden /> Agent panel
          </div>
          <h1 className="text-3xl font-semibold tracking-normal text-bolt-elements-textPrimary sm:text-4xl">
            A Replit-style copilot wired to the preserved Bolt IDE
          </h1>
          <p className="mt-3 max-w-2xl text-sm text-bolt-elements-textSecondary">
            The VibeCore IDE agent lives next to your editor. It accepts @ file mentions and / slash commands, streams a
            plan-first checklist, hands you per-hunk patch review, archives conversation branches and shows live
            presence — all in the same panel, with no separate copilot tab. Below is a feature-by-feature walkthrough
            with screenshots, prerequisites and the keystroke you actually press.
          </p>
          <div className="bolt-feature-doc-hero-actions">
            <LinkButton to="/sign-up">Get started</LinkButton>
        ...
        <aside className="bolt-feature-doc-prereqs" aria-label="Prerequisites">
          <h2 className="text-base font-semibold text-bolt-elements-textPrimary">Before you start</h2>
          <ul>
            {/* eslint-disable-next-line @typescript-eslint/naming-convention -- Icon is destructured into a component-style alias for JSX rendering */}
            {PREREQUISITES.map(({ icon: Icon, title, detail }) => (
              <li key={title}>
                <Icon className="h-4 w-4 text-bolt-elements-icon-success" aria-hidden />
                <div>
        ...
        <FeatureSection
          ...
          steps={[
            {
              label: 'Type @ in the composer',
              detail: 'The palette opens anchored above the textarea. No mouse needed.',
            },
            {
              label: 'Filter with letters',
              detail: 'Fuzzy match across path + basename. Recent picks float to the top.',
            },
            {
        ...
        <FeatureSection
          ...
          notes={
            <p>
              <strong>Standalone Bolt safety:</strong> commands that need a project context (/snapshot, /run,
        ...
        <FeatureSection
          ...
          steps={[
            ...
            },
            {
              ...
              detail:
                'Items flip done / in-progress / failed as the agent reports back, with strikethrough on completion.',
            },
          ]}
        ...
        <FeatureSection
          ...
          notes={
            <p>
              <strong>Auto-apply ON?</strong> The review panel is hidden — writes flow straight to the workbench with
              the existing Undo toast. The agent UX feels native, not gated.
            </p>
          }
        />
        ...
        <footer className="bolt-feature-doc-footer">
          <h2 className="text-lg font-semibold text-bolt-elements-textPrimary">Ready to try?</h2>
          <p className="text-sm text-bolt-elements-textSecondary">
            Spin up a project, open the IDE and try <code>@</code> or <code>/</code> in the agent composer. Every
            feature above is on by default — no flags, no extra setup.
          </p>
          <div className="bolt-feature-doc-hero-actions">
            <LinkButton to="/sign-up">Create your workspace</LinkButton>
