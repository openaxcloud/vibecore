...
describe('the public marketing surfaces stay legible in both themes', () => {
  ...
  /*
   * Les TROIS états de thème, pas seulement `:root`. La version précédente de ce
   * test ne mesurait que `:root` et passait dans les deux sens — remettre
   * `--ecode-text-muted: var(--vc-ide-text-muted)` la laissait verte. Elle ne
   * gardait donc rien.
   *
   * L'état non stampé compte autant que les deux autres : c'est le réglage par
   * défaut du navigateur, celui que voit la majorité des visiteurs, et il ne
   * porte NI `[data-theme='light']` NI `[data-theme='dark']`.
   */
  it.each([
    ['non stampé (réglage système)', ':root'],
    ['sombre explicite', ":root[data-theme='dark']"],
    ['clair explicite', ":root[data-theme='light']"],
  ])('garde le gris discret du marketing lisible sur sa surface surélevée — %s', (_état, sélecteur) => {
    const muted = token(sélecteur, '--ecode-text-muted');
    const surface = token(sélecteur, '--ecode-surface-tertiary');

    expect(contrast(muted, surface)).toBeGreaterThanOrEqual(AA_BODY_TEXT);
  });

  /*
   * Le gris discret doit rester DISCRET : sans cette borne, on pourrait passer
   * le test précédent en le rapprochant du texte secondaire, et la hiérarchie
   * visuelle disparaîtrait au profit du seul contraste.
   */
  it.each([
    ['non stampé (réglage système)', ':root'],
    ['sombre explicite', ":root[data-theme='dark']"],
    ['clair explicite', ":root[data-theme='light']"],
  ])('garde le gris discret en retrait du texte secondaire — %s', (_état, sélecteur) => {
    const surface = token(sélecteur, '--ecode-surface-tertiary');
    const muted = contrast(token(sélecteur, '--ecode-text-muted'), surface);
    const secondary = contrast(token(sélecteur, '--ecode-text-secondary'), surface);
