...
describe('TINT-SWEEP-001 — texte sur une teinte de sa propre couleur', () => {
  it('la résolution distingue bien les deux thèmes', () => {
     ...
     *
     * ⚠️ Cette garde a elle-même eu le défaut qu'elle prétendait couvrir : elle
     * RECOPIAIT les deux préfixes en dur au lieu de lire `THEMES`. Mettre un
     * préfixe cité dans `THEMES` — le mode de panne décrit plus haut, et celui
     * qu'on obtient en recopiant la source SCSS, qui elle cite — laissait donc
     * les 4 tests au vert tout en mesurant le thème SOMBRE deux fois. Vérifié :
     * avec `:root[data-theme='light']`, `--vc-ide-bg-panel` retombe sur `:root`
     * et vaut `#0e1525`, pas `#ffffff`. La garde lit désormais `THEMES`, donc
     * elle tombe avec la sonde qu'elle garde.
     */
    for (const theme of THEMES) {
      expect(BLOCKS.has(theme.prefixe), `aucun bloc pour le préfixe ${theme.prefixe}`).toBe(true);
    }

    const [clair, sombre] = THEMES.map((t) => jeton(t.prefixe, '--vc-ide-bg-panel'));
