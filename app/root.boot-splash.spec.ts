...
describe('pre-hydration E-Code splash', () => {
  ...
  it('uses the branded fallback for every pre-hydration route, including marketing', () => {
    expect(rootSource).toContain('<ClientOnly fallback={<AppBootFallback ide={showIdeBootFallback} />}>');
    expect(rootSource).toContain('<DndProvider backend={HTML5Backend}>{children}</DndProvider>');
  });
