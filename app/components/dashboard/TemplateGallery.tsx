...
export type GalleryReportReason =
  | 'COPYRIGHT'
  | 'DECEPTIVE'
  | 'HARMFUL'
  | 'INAPPROPRIATE'
...
export type GalleryReportRequest = {
  reason: GalleryReportReason;
  details?: string;
  submissionId: string;
};

export type GalleryReportFeedback = {
  appId: string;
  submissionId: string;
...
type TemplateGalleryProps = {
  ...
  onReport?: (app: GalleryApp, report: GalleryReportRequest) => Promise<'deferred' | void> | 'deferred' | void;
  onRetry?: () => void;
  onView?: (app: GalleryApp) => void;
  nextPageHref?: string | null;
  reportFeedback?: GalleryReportFeedback | null;
...
export function TemplateGallery({
  ...
  const [reportTarget, setReportTarget] = useState<GalleryApp | null>(null);
  ...
  const facets = useMemo(
    () => globalFacets ?? templateGalleryFacets(apps, includeUnapproved),
    [apps, globalFacets, includeUnapproved],
  );
  ...
  useEffect(() => {
    if (!reportTarget || !reportSubmissionId || reportFeedback?.submissionId !== reportSubmissionId) return;

    if (reportFeedback.status === 'error') {
      setReportError(reportFeedback.error ?? `Could not report ${reportTarget.name}.`);
      return;
    }

    if (reportFeedback.status === 'success') {
      setReportTarget(null);
      setReportSubmissionId(null);
      setReportError(null);
      queueMicrotask(() => reportTriggerRef.current?.focus());
    }
  }, [reportFeedback, reportSubmissionId, reportTarget]);

  const openReport = (app: GalleryApp, trigger: HTMLButtonElement) => {
    if (!onReport || pendingReportId || reportFeedback?.status === 'pending') return;
  ...
  const closeReport = () => {
    if (pendingReportId || reportFeedback?.status === 'pending') return;
    setReportTarget(null);
    setReportSubmissionId(null);
    setReportError(null);
    queueMicrotask(() => reportTriggerRef.current?.focus());
  ...
  const report = async (app: GalleryApp, input: Omit<GalleryReportRequest, 'submissionId'>) => {
    if (!onReport || pendingReportId) {
      return;
    }

    const submissionId = createReportSubmissionId();
    setReportSubmissionId(submissionId);
    setReportError(null);
    setPendingReportId(app.id);

    try {
      const result = await onReport(app, { ...input, submissionId });
      if (result !== 'deferred') {
        setReportTarget(null);
        setReportSubmissionId(null);
        queueMicrotask(() => reportTriggerRef.current?.focus());
      ...
      setReportError(errorMessage(caughtError, `Could not report ${app.name}.`));
    } finally {
    ...
    >
          ...
          <div className={classNames(view === 'grid' ? 'grid gap-4 sm:grid-cols-2 2xl:grid-cols-3' : 'grid gap-3')}>
            {visibleApps.map((app, index) => (
              <GalleryAppCard
                ...
                onRequestReport={onReport ? openReport : undefined}
                onView={onView}
                remixPending={activeRemixId === app.id}
                remixLocked={Boolean(activeRemixId)}
                reportPending={
                  pendingReportId === app.id ||
                  (reportFeedback?.appId === app.id && reportFeedback.status === 'pending')
                }
              />
            ))}
          </div>
      ...
      {reportTarget ? (
        <GalleryReportDialog
          ...
          error={reportError}
          pending={
            pendingReportId === reportTarget.id ||
            (reportFeedback?.appId === reportTarget.id &&
              reportFeedback.submissionId === reportSubmissionId &&
              reportFeedback.status === 'pending')
          }
          onCancel={closeReport}
          onSubmit={(input) => report(reportTarget, input)}
        />
      ) : null}
    </section>
  );
...
const REPORT_REASON_OPTIONS: readonly {
...
}[] = [
  {
    ...
    description: 'The app appears to use protected work without permission.',
  },
  {
    value: 'DECEPTIVE',
    label: 'Deceptive or misleading',
    description: 'The listing or app misrepresents its purpose, behavior or provenance.',
  },
  {
    value: 'HARMFUL',
    label: 'Harmful content',
    description: 'The app promotes harm or creates a credible safety risk.',
  },
  {
    value: 'INAPPROPRIATE',
    label: 'Inappropriate content',
    description: 'The app contains content that is not suitable for this Gallery.',
  },
  {
    value: 'MALWARE',
    label: 'Malware or unsafe code',
    description: 'The app appears to contain malicious or intentionally unsafe behavior.',
  },
  {
    value: 'PRIVACY',
    label: 'Privacy violation',
    description: 'The app exposes personal information or handles it without permission.',
  },
  {
    value: 'SPAM',
    label: 'Spam',
    description: 'The app is repetitive, irrelevant or primarily promotional abuse.',
  },
  {
    value: 'OTHER',
    label: 'Another issue',
    description: 'Describe an issue that is not covered by the reasons above.',
  },
];

function GalleryReportDialog({
  app,
  error,
  onCancel,
  onSubmit,
  ...
  error: string | null;
  onCancel: () => void;
  onSubmit: (input: Omit<GalleryReportRequest, 'submissionId'>) => Promise<void> | void;
  pending: boolean;
}) {
  const [reason, setReason] = useState<GalleryReportReason | ''>('');
  const [details, setDetails] = useState('');
  const dialogRef = useRef<HTMLDivElement | null>(null);
  const selectedReason = REPORT_REASON_OPTIONS.find((option) => option.value === reason);
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const focusable = [
        ...(dialogRef.current?.querySelectorAll<HTMLElement>('button, select, textarea') ?? []),
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/65 p-3 backdrop-blur-sm sm:p-6">
      <div
        ...
        aria-describedby="gallery-report-description"
        aria-busy={pending || undefined}
        className="max-h-[calc(100dvh-1.5rem)] w-full max-w-lg overflow-y-auto rounded-2xl border border-bolt-elements-borderColor bg-bolt-elements-background-depth-2 shadow-[var(--vc-ui-shadow-xl)] sm:max-h-[calc(100dvh-3rem)]"
      >
        <div className="flex items-start justify-between gap-4 border-b border-bolt-elements-borderColor px-4 py-4 sm:px-5">
          <div className="min-w-0">
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-bolt-elements-textTertiary">
              Moderation report
            </p>
            <h2
              id="gallery-report-title"
              className="mt-1 truncate text-lg font-semibold text-bolt-elements-textPrimary"
            >
              Report {app.name}
            </h2>
            <p id="gallery-report-description" className="mt-1 text-sm leading-5 text-bolt-elements-textSecondary">
              Choose the closest reason. The moderation team will review the app; this does not remove it automatically.
            </p>
          </div>
          <button
            type="button"
            ...
            className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-md text-bolt-elements-textTertiary transition-colors hover:bg-bolt-elements-background-depth-3 hover:text-bolt-elements-textPrimary focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--vc-ide-accent-action)] disabled:cursor-wait disabled:opacity-50"
          >
            <X className="h-4 w-4" aria-hidden />
          </button>
        </div>
        ...
        <form
          ...
          onSubmit={(event) => {
            event.preventDefault();
            if (!reason || !canSubmit || pending) return;
            const normalizedDetails = details.trim();
            void onSubmit({ reason, ...(normalizedDetails ? { details: normalizedDetails } : {}) });
          }}
        >
          {error ? (
            <div
              role="alert"
              className="rounded-lg border border-[var(--status-error-border)] bg-[var(--status-error-bg)] px-3 py-2.5 text-sm text-[var(--status-error-text)]"
            >
              <p className="font-semibold">The report was not sent</p>
              <p className="mt-1">{error} Correct the form or try again.</p>
            </div>
          ) : null}

          <label className="block" htmlFor="gallery-report-reason">
            <span className="mb-1.5 block text-sm font-semibold text-bolt-elements-textPrimary">Reason</span>
            <span className="relative block">
              <select
                ...
                disabled={pending}
                className="h-11 w-full appearance-none rounded-md border border-bolt-elements-borderColor bg-bolt-elements-background-depth-1 pl-3 pr-9 text-sm text-bolt-elements-textPrimary outline-none focus-visible:border-[var(--vc-ide-accent-action)] focus-visible:ring-2 focus-visible:ring-[var(--vc-ide-focus-ring)] disabled:opacity-60"
              >
          ...
          <p className="min-h-5 text-xs leading-5 text-bolt-elements-textTertiary" aria-live="polite">
            {selectedReason?.description ?? 'Select the issue that best matches what you observed.'}
          </p>

          <label className="block" htmlFor="gallery-report-details">
            <span className="mb-1.5 flex items-center justify-between gap-3 text-sm font-semibold text-bolt-elements-textPrimary">
              <span>Details {detailsRequired ? '(required)' : '(optional)'}</span>
              <span className="text-xs font-normal text-bolt-elements-textTertiary">{details.length}/1000</span>
            </span>
            <textarea
              ...
              rows={4}
              disabled={pending}
              placeholder="Share only the information moderators need. Do not include secrets or personal data."
              className="w-full resize-y rounded-md border border-bolt-elements-borderColor bg-bolt-elements-background-depth-1 px-3 py-2.5 text-sm leading-5 text-bolt-elements-textPrimary outline-none placeholder:text-bolt-elements-textTertiary focus-visible:border-[var(--vc-ide-accent-action)] focus-visible:ring-2 focus-visible:ring-[var(--vc-ide-focus-ring)] disabled:opacity-60"
            />
          </label>

          <div className="flex flex-col-reverse gap-2 border-t border-bolt-elements-borderColor pt-4 sm:flex-row sm:justify-end">
            <Button type="button" variant="outline" className="min-h-11 px-4" onClick={onCancel} disabled={pending}>
              Cancel
            </Button>
            <Button type="submit" variant="primary" className="min-h-11 px-4" disabled={!canSubmit || pending}>
              {pending ? (
                <LoaderCircle className="mr-2 h-4 w-4 animate-spin motion-reduce:animate-none" aria-hidden />
              ) : (
...
function GalleryAppCard({
  ...
  list: boolean;
  onRemix?: (app: GalleryApp) => Promise<void> | void;
  onRequestReport?: (app: GalleryApp, trigger: HTMLButtonElement) => void;
  onView?: (app: GalleryApp) => void;
  remixLocked: boolean;
    ...
    >
      ...
      >
        ...
        {onRequestReport ? (
          <button
            type="button"
            onClick={(event) => onRequestReport(app, event.currentTarget)}
...
let fallbackReportSubmissionSequence = 0;

function createReportSubmissionId() {
  if (typeof globalThis.crypto?.randomUUID === 'function') {
    return `gallery-report-${globalThis.crypto.randomUUID()}`;
  }

  fallbackReportSubmissionSequence += 1;
  return `gallery-report-${Date.now()}-${fallbackReportSubmissionSequence}`;
}
