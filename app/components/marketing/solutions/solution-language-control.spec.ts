...
describe('Solutions language control ownership', () => {
  ...
  it('keeps the shared public header as the single visible language control', () => {
  ...
  it('removes the obsolete hero-level switch styles from both Solutions layouts', () => {
    const appBuilderStyles = readSource('app/components/marketing/solutions/app-builder.css');
    const sharedStyles = readSource('app/components/marketing/solutions/solution-sales.css');

    expect(appBuilderStyles).not.toContain('.app-builder-language-switch');
    expect(sharedStyles).not.toContain('.sol-language-switch');
  });
});
