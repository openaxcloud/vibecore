...
import { loader as blogLoader, meta as blogMeta } from '~/routes/blog';
...
describe('exact legal registry and blog catalogs', () => {
  ...
  it('localizes legal and Blog SEO, including social image alternatives', () => {
    ...
    expect(blogTags).toEqual(expect.arrayContaining([{ property: 'og:image:alt', content: blogSeo.imageAlt }]));
    expect(blogTags).toEqual(expect.arrayContaining([{ name: 'twitter:title', content: blogSeo.title }]));
    expect(blogTags).toEqual(expect.arrayContaining([{ name: 'twitter:description', content: blogSeo.description }]));
  });
