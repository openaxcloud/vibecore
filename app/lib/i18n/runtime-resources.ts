/**
 * LES RESSOURCES STATIQUES de i18next — les 150 catalogues `…En` / `…Fr`
 * fusionnés dans un seul espace de noms `translation` par langue.
 *
 * CE MODULE NE DOIT JAMAIS ÊTRE ATTEIGNABLE DEPUIS LE CLIENT. C'est le défaut
 * BUG-PERF-I18N-RACINE-001, mesuré le 2026-09-14 : quand `runtime.ts` portait
 * ces imports, `root.tsx` (qui appelle `createI18nInstance`) traînait les deux
 * langues ENTIÈRES sur toute page — 653 Ko de texte anglais + 738 Ko de texte
 * français sur la page d'accueil marketing d'un visiteur qui n'ouvrira jamais
 * l'IDE. Le chunk `runtime-*.js` pesait 979 Ko à lui seul.
 *
 * Qui a le droit d'importer ce module, et pourquoi :
 *   - `app/entry.server.tsx` — le serveur enregistre tout au démarrage, le
 *     rendu SSR a besoin des quatre langues en synchrone ;
 *   - `app/lib/i18n/catalogues-pour-vitest.ts` — le fichier de setup des tests,
 *     pour que les 127 specs qui montent un `I18nextProvider` restent inchangées ;
 *   - `build-config/catalogues-i18n-plugin.ts` — le build ÉVALUE ce module et
 *     émet un JSON par langue ; le navigateur ne charge que le sien.
 *
 * La liste est tenue par `runtime-resources.spec.ts` : un import ailleurs dans
 * `app/` fait rougir la garde AVANT que le manifeste ne regrossisse.
 */
...
/** Le catalogue plat d'une langue — ce que le build sérialise en JSON. */
export function catalogueDeLaLangue(langue: SupportedLanguage): Record<string, string> {
  return RESOURCES[langue].translation;
}
