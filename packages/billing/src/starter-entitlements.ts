...
/**
 * Cap de projets publiés ACTIFS par plan. Starter vient de la rate card ; les
 * plans payants sont bornés par la limite dure « 20 apps simultanées », qui
 * s'applique à tous les plans et n'est donc pas un quota commercial Starter.
 */
export function maxActivePublishedProjects(plan: EntitlementPlanKey): number {
  ...
  return usableValue(STARTER_RATE_CARD.technicalLimits.concurrentAppsAllPlans) ?? 0;
}
...
export function evaluatePublish(input: {
  ...
  const cap = Number.isFinite(rawCap) && rawCap >= 0 ? rawCap : 0;
