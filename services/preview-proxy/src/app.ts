...
export function parsePublishedBadgeFrameHost(
  ...
  const host = hostHeader.split(':')[0].trim().toLowerCase();
  const normalizedDomain = previewDomain.trim().toLowerCase().replace(/^\.+|\.+$/g, '');
  ...
  return match
    ? { kind: match[1] === 'rd' ? 'server' : 'static', deploymentId: match[2]! }
    : null;
}
...
export async function buildPreviewProxyApp(options: PreviewProxyOptions = {}): Promise<FastifyInstance> {
  ...
  const rawPublishedFrameUrl = (
    ...
    const domain = previewDomain.trim().toLowerCase().replace(/^\.+|\.+$/g, '');
    ...
    app.addHook('onRequest', async (request, reply) => {
      ...
      if (badgeFrame) {
        ...
        if (verdict.state !== 'live' || !verdict.badgeRequired) {
          reply.code(verdict.state === 'expired' ? 410 : 404).header('cache-control', 'private, no-store').send();
          return;
        }
        ...
        if (badgeFrame.kind === 'server') {
          await handleServerDeployRequest(
            request,
            reply,
