...
export class PrismaApiStore implements ApiStore, ReservedVmBillingStore {
  ...
  private async lockExpectedProjectTenantMutation(
    ...
    if (
      scope.accountPurgeCoordinator &&
      project.ownershipEpoch !== scope.accountPurgeCoordinator.expectedOwnershipEpoch
    ) {
      throw projectOrganizationChangedError();
    }
  ...
  async hardDeleteProject(
    ...
    if (!deletionAuthority) {
      throw Object.assign(new Error('Permanent deletion authority is required'), {
    ...
    return this.withProjectPhysicalBarrier(input.projectId, async () => {
      ...
      return this.withProjectFilesystemLock(input.projectId, async () => {
        ...
        const heartbeat = this.objectStorageOperationHeartbeat(claimed.lease);
        const assertDeletionAuthority = async () => {
          await heartbeat.assert();
            ...
            if (!resumePersistedVolumeBatch) {
              ...
              try {
                ...
                databasePlan = await this.prisma.$transaction(async (tx) => {
                  await recordPermanentDeletionStaticArtifactPlan(tx, heartbeat.lease(), staticArtifactPlan);
