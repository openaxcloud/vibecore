...
export async function fencePurgedUserStateMachines(
  ...
  await assertAccountPurgeProjectChildrenComplete(tx, partialProjectIds);
  /* Partial import/remix projects are still full Project aggregates. The
   * parent purge must run their canonical permanent-delete child saga too;
   * deleting a merely soft-deleted row here would bypass its physical proof. */
  const partialProjectsDeleted = 0;
