...
/** `sha256:<hex>` → tag-schema tag `sha256-<hex>` (OCI referrers fallback). */
...
export class OciDistributionAdapter implements RegistryAdapter {
  ...
  /**
   * Delete the image AND its attachments (coupled retention, §13.5): remove the
   * attestations first so a rollback never orphans them, then the image itself.
   * Best-effort — a missing target is a no-op.
   */
  async deleteImageAndReferrers(repo: string, digest: string): Promise<void> {
    ...
    // Coupled retention (§13.5), ordered for registries that enforce referential
    // integrity (e.g. zot returns 405 when deleting a manifest still referenced
    // by a tagged index):
    //   1. the tag-schema fallback index (releases its references to the
    //      attachment manifests),
    //   2. every attachment/attestation (now unreferenced),
