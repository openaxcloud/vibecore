...
export function projectCheckpointAdmissible(
  ...
  if (kinds.size === 1 && kinds.has('POD')) {
    return { admissible: false, reason: 'un snapshot de pod seul n\'est jamais un checkpoint projet (plan §15)' };
  }

  if (!kinds.has('FILES')) {
    return { admissible: false, reason: 'composant FILES manquant' };
  }

  if (opts.databaseProvisioned && !kinds.has('DATABASE') && opts.databaseDependencyDeclared !== true) {
    return {
      admissible: false,
      reason: 'base provisionnée mais composant DATABASE absent sans dépendance déclarée',
    };
  }
