...
export function guardObjectStorageWrites(
  ...
  return {
    ...
    /*
     * LECTURE : relayée sans passer par `refuseIfFrozen`, comme `listObjects`.
     * Cette enveloppe garde les ÉCRITURES pendant un gel de purge ; refuser
     * l'inventaire ici empêcherait justement la purge de savoir ce qu'elle doit
     * effacer. Arrivée sur `main` après l'écriture de cette enveloppe.
     */
    listAllObjects: (projectId, opts) => inner.listAllObjects(projectId, opts),
