...
describe('Cloud Run tenant capacity (claim GCP-15)', () => {
  ...
  it('NEGATIVE: a shard project at its utilisation ceiling refuses the next service', () => {
    ...
    expect(refused.reasonCode).toBe('CAPACITY_CEILING_REACHED');
    expect(refused.details).toEqual({
      currentServicesInShard: 800,
      ceiling: 800,
      maxServicesPerProjectRegion: 1000,
      serviceUtilisationCeiling: 0.8,
    });
  });

  it('MUTATION: NaN, negative and over-100% inputs fail closed instead of admitting placement', () => {
    expect(admitServicePlacement(Number.NaN)).toMatchObject({ admissible: false });
    expect(admitServicePlacement(-1)).toMatchObject({ admissible: false });
    expect(
      admitServicePlacement(0, {
        ...DEFAULT_TENANT_CAPACITY_POLICY,
        serviceUtilisationCeiling: 1,
      }),
    ).toMatchObject({ admissible: false });
    expect(() => requiredProjectShards(Number.NaN)).toThrow(/serviceCount/);
  });
});
...
describe('pre-created project pool (claim GCP-14)', () => {
  ...
  it('MUTATION: corrupt pool state is rejected rather than normalized into capacity', () => {
    expect(() => planProjectPool({ free: -1 })).toThrow(/free project count/);
    expect(() => planProjectPool({ free: Number.NaN })).toThrow(/free project count/);
  });
});
