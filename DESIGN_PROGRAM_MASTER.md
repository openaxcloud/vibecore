...
| ID | Point | 📤 Dispatché | 💻 Codé | ✅ Testé live | Preuve |
|---|---|:---:|:---:|:---:|---|
| TPL-02.1 | Requalifier `/dashboard/templates` en Gallery d'applications communautaires publiées : carte riche, recherche, catégories, type d'artefact, technologies, tri, mise en avant, modération, signalement, aperçu fonctionnel et contrôle du remix | ✅ | ✅ `266fefac`/`e6afdfbf`/`c59674e8`/`3181b31f` | 🟡 fonctionnel prouvé | Fonctionnel LIVE prod : curation admin 201 → browse anonyme (`GET /gallery`=200) → détail (vues 0→3) → grille+détail UI (desktop+mobile). `docs/deploy-evidence/2026-07-17-gallery/README.md` + `GALLERY_COMMUNITY_CONTRACT.md §C`. **Design non ✅** : la matrice clair/sombre 390/768/1024/1440 n'est pas capturée (cf. `DESIGN_AUDIT_LIVE.md` TPL-02.1 ☐). « aperçu embarqué » = décision E-CODE de NE PAS faire (lien sortant, `UNK-GALLERY-EMBED-PREVIEW`). |
| TPL-02.2 | Remix/Fork isolé : nouveau projet/propriétaire/repo/workspace/locks, copie fichiers+config, aucun secret, données isolées, provenance source et analyse Agent post-remix | ✅ | ✅ `266fefac`/`3181b31f` | 🟡 core prouvé | Prouvé LIVE : remix par un AUTRE user → clone dans l'org du remixeur, `RemixJob COMPLETED` épinglé `sourceSnapshotId`+`sourceListingId`, clone=7 fichiers, `secrets:[]` live, `useCount=1`. **Reste** : clic-connecté « Remix » → IDE navigateur (handoff Chrome d'Avi) ; secret-absent exhaustif = test `gallery-routes.spec.ts`. |
